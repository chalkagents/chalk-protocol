...
work in your favor: there are **zero dependencies** (`npm test` runs the complete `node --test`
suite with bounded concurrency, and there is nothing to install), and
...
Fix, add a test that **fails without your change**, open a PR. CI runs the same complete
`node --test` suite with bounded concurrency on every PR.
