...
const calls = (path) => +readFileSync(path, 'utf8'); // how many times the stub reviewer ran
...
function project(reviewerBody) {
  ...
  const counter = `${d}.calls`; // outside the read-only reviewer workspace
  chalk(d, 'init', '--name', 'demo');
  writeFileSync(join(d, 'reviewer.mjs'), reviewerBody.replace("const c = 'n.txt'", `const c = ${JSON.stringify(counter)}`));
...
test('review — a transient failure is retried once, then the valid verdict is accepted', () => {
  const { d, id, counter } = project(FLAKY);
  ...
  assert.equal(calls(counter), 2, 'exactly two reviewer invocations — the initial call plus one retry');
});

test('review — two consecutive errors are fatal, and the retry is BOUNDED (not infinite/swallowed)', () => {
  const { d, id, counter } = project(ALWAYS_ERR);
  const r = chalk(d, 'review', id);
  assert.notEqual(r.code, 0, 'a persistent reviewer failure still fails the review — it is not swallowed');
  assert.equal(calls(counter), 2, 'retried exactly once (2 calls total), then dies — bounded, not indefinite');
});
...
test('review --no-retry — suppresses the retry (the pipeline owns stage-level retry, must not double-count)', () => {
  ...
  assert.equal(calls(counter), 1, 'exactly one reviewer call — --no-retry suppresses the inner retry');
});
