...
export function nextActionView(tasks = []) {
  ...
  if (wip[0]) primary = {
    ...
    const dependency = (waiting[0].after || []).map((ref) => resolveRef(tasks, ref)).find((task) => task && task.state !== 'done');
  ...
  else if (tasks.every((task) => task.state === 'done')) primary = { kind: 'done', task: null, command: 'chalk task add "<title>"', label: 'All tasks are done; add the next one', details: [] };
  ...
  return {
    ...
    counts: {
      ...
      dependencies: waiting.length, done: tasks.filter((task) => task.state === 'done').length,
    },
  };
}
