...
function git(root, args, allowed = [0]) {
  const r = launchCommand('git', ['--no-optional-locks', '-c', 'diff.autoRefreshIndex=false', '-c', 'core.warnAmbiguousRefs=true', ...args], {
    cwd: root, encoding: 'utf8', timeout: 30000, maxBuffer: 64 * 1024 * 1024, env: { ...process.env, LC_ALL: 'C' },
  });
