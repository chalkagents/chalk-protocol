...
function git(root, args, allowed = [0], input) {
  const r = launchCommand('git', ['--no-replace-objects', '--no-optional-locks', '-c', 'diff.autoRefreshIndex=false', '-c', 'core.warnAmbiguousRefs=true', '-c', 'core.filemode=true', ...args], {
...
function checkParents(root, path) {
  ...
  for (let i = 1; i < parts.length; i++) {
    try { if (lstatSync(resolve(root, ...parts.slice(0, i))).isSymbolicLink()) throw new Error('symlinked review input directory'); }
...
export function captureReviewInputs(root, task, protocol = {}) {
  ...
  const excludes = [...SPINE_STATE_PATHS, '.chalk/local', '.chalk/.lock', '.chalk/.locks', '.chalk/held-out'];
  const realRoot = realpathSync(root);
  for (const protectedRoot of [resolve(root, '.chalk/held-out'), resolve(root, String(protocol.regression?.dir || '.chalk/held-out').replace(/^~(?=$|[/\\])/, homedir()))]) {
    let canonical;
    try { canonical = realpathSync(protectedRoot); } catch (error) { if (error.code !== 'ENOENT') throw error; canonical = protectedRoot; }
    for (const path of [relative(root, protectedRoot), relative(realRoot, canonical)]) {
      const normalized = path.split('\\').join('/');
      if (!normalized || /^(?:\.\.\/)*\.\.$/.test(normalized)) throw new Error('protected regression directory covers the review workspace');
      if (!normalized.startsWith('../')) excludes.push(normalized);
    }
  }
  ...
  for (const path of visible) checkParents(root, path);
  if (visible.length) {
