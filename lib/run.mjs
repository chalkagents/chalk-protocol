...
export function runDriver(store, { until = 'empty', max = 50, dryRun = false, reviewRequiredNow = () => false, log = () => {} } = {}) {
  ...
  while (iterations < max) {
    ...
    if (reviewRequiredNow(store, store.task(t.id))) {
      ...
      const r = runReview(store, reviewedTask, { onInputs: input => log(formatReviewInputs(input)) });
      ...
      if (r.status === 'ok') {
        cur.reviews.push({ at: now(), by: 'adversary', verdict: r.verdict, findings: r.findings, specRevision: reviewedRevision, approval: r.approval, inputs: r.inputs });
