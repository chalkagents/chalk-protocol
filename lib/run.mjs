...
export function runDriver(store, { until = 'empty', max = 50, dryRun = false, reviewRequiredNow = () => false, log = () => {} } = {}) {
  ...
  if (!executorProfile?.command) return { degraded: true, next: planned[0] || null };
  ...
  while (iterations < max) {
    ...
    if (!verify(store, { cwd: workdir(store, beforeVerify) }).green) {
    ...
    if (reviewRequiredNow(store, store.task(t.id))) {
      ...
      const r = runReview(store, reviewedTask);
      ...
      if (r.status === 'ok') {
        cur.reviews.push({ at: now(), by: 'adversary', verdict: r.verdict, findings: r.findings, specRevision: reviewedRevision });
    ...
    const remainingApprovals = completionApprovalBlockers(store, done);
    if (remainingApprovals.length || (done.specRevision || 0) !== verifiedRevision) {
      blockTask(store, done, remainingApprovals.length ? remainingApprovals.join('; ') : 'specification changed after verification — reload context and retry', remainingApprovals.length ? 'human-input' : 'review');
      blocked.push(t.id);
      if (until === 'blocked') { stopped = 'blocked'; break; }
      continue;
    }
