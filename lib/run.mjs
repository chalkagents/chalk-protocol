...
export function runDriver(store, { until = 'empty', max = 50, dryRun = false, reviewRequiredNow = () => false, log = () => {} } = {}) {
  ...
  while (iterations < max) {
    ...
    let verification = verify(store, { cwd: workdir(store, beforeVerify) });
    ...
    if (!bi.skipped || !mut.skipped) {
      verification = verify(store, { cwd: workdir(store, store.task(t.id)) });
      if (!verification.green) {
    ...
    if (reviewRequiredNow(store, store.task(t.id))) {
      ...
      if (r.status === 'ok') {
        cur.reviews.push({ at: now(), by: 'adversary', verdict: r.verdict, findings: r.findings, specRevision: reviewedRevision, approval: r.approval });
      ...
      if (!(r.status === 'ok' && r.verdict === 'pass')) {
        ...
        const inputFailure = r.diagnostics?.find(diagnostic => diagnostic.code === 'approval-inputs');
