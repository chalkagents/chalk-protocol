...
import { verifyAfterExternalGates } from './verification-boundary.mjs';
...
export function runDriver(store, { until = 'empty', max = 50, dryRun = false, reviewRequiredNow = () => false, log = () => {} } = {}) {
  ...
  while (iterations < max) {
    ...
    const reviewRan = reviewRequiredNow(store, store.task(t.id));
    if (reviewRan) {
    ...
    // Break-it, mutation, and review all invoke external commands after the first
    // verification. Only advance on a receipt bound to their final source state.
    const finalVerification = verifyAfterExternalGates({
      gates: [bi, mut], externalRan: reviewRan,
      verify: () => verify(store, { cwd: workdir(store, store.task(t.id)) }),
    });
