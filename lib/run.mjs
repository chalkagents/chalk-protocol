...
import { runReview, runReviewWithRetry, formatReviewFailure } from './review.mjs';
...
import { finishTarget, formatFinishTarget, finishTrackingBlocker, finishPolicyIdentity, finishContextIdentity, finishProbeScope, finishContractCurrent, validateFinishVerification } from './run-finish.mjs';
...
export function runDriver(store, { until = 'empty', max = 50, dryRun = false, finish, forceRerun = false, reviewRequiredNow = () => false, log = () => {} } = {}) {
  ...
  const initialReviewRequired = Boolean(selected && reviewRequiredNow(store, selected));
  const needsReview = task => initialReviewRequired || reviewRequiredNow(store, task);
  ...
  while (iterations < max) {
    ...
    if (!selected) {
      ...
      try { requiredProbes = finishProbeScope(store, t) || {}; }
      catch (error) {
    ...
    const initialFinishPolicy = selected ? finishPolicyIdentity(store) : null;
    const initialFinishContext = selected ? finishContextIdentity(store, beforeVerify) : null;
    ...
    const bi = runBreakit(store, store.task(t.id), { strictExecution: Boolean(selected) });
    if (selected && requiredProbes.breakTest && bi.skipped) {
      blockTask(store, store.task(t.id), `configured break-it probe did not run: ${bi.reason}`, 'review');
      blocked.push(t.id); stopped = 'blocked'; break;
    }
    ...
    const mut = runMutation(store, store.task(t.id), { strictExecution: Boolean(selected) });
    if (selected && requiredProbes.mutation && mut.skipped) {
      blockTask(store, store.task(t.id), `configured mutation probe did not run: ${mut.reason}`, 'review');
      blocked.push(t.id); stopped = 'blocked'; break;
    }
    ...
    if (!bi.skipped || !mut.skipped) {
      if (selected) {
        const initial = finishContractCurrent(store, store.task(t.id), initialVerificationApproval, initialFinishPolicy, initialFinishContext);
        if (!initial.current) {
    ...
    if (needsReview(store.task(t.id))) {
      const reviewedTask = store.task(t.id);
      const reviewedRevision = reviewedTask.specRevision || 0;
      let reviewAttempt = 0;
      const r = runReviewWithRetry(
        () => timed(t, reviewAttempt++ ? 'review retry' : 'review', () => runReview(store, reviewedTask, { onInputs: input => log(formatReviewInputs(input)) })),
        { onRetry: () => log('review: documented transient failure; retrying once') },
      );
      if (selected) {
      ...
      if (r.status === 'ok') {
        cur.reviews.push({ at: now(), by: 'adversary', verdict: r.verdict, findings: r.findings, decisions: r.decisions || [], specRevision: reviewedRevision, approval: r.approval, inputs: r.inputs, invocation: r.invocation });
      ...
      if (!(r.status === 'ok' && r.verdict === 'pass')) {
        ...
        blockTask(store, store.task(t.id), inputFailure?.message || (r.status === 'ok' ? `review ${r.verdict}` : formatReviewFailure(r)), r.status === 'ok' || inputFailure ? 'review' : 'human-input');
        blocked.push(t.id);
        if (until === 'blocked') { stopped = 'blocked'; break; }
        continue;
      }
    ...
    store.mutateTasks(tasks => {
      ...
      if (needsReview(done) && !currentReview(store, done)) { remainingApprovals.push(`review approval is stale or historical — run chalk review ${done.id}`); admissionNeeds = 'review'; }
      ...
      if (selected) {
        const initial = finishContractCurrent(store, done, initialVerificationApproval, initialFinishPolicy, initialFinishContext);
        if (!initial.current) remainingApprovals.push(`initial verification inputs changed: ${initial.reason}`);
        if (openRaises(done).length) { remainingApprovals.push('unanswered director forks — answer via chalk pending before finishing'); admissionNeeds = 'decision'; }
        if (missingRequiredTest(store, done)) remainingApprovals.push('no required test remains in the change');
        if (!depsSatisfied(done, tasks) || !depsSatisfied(selected, tasks)) remainingApprovals.push('task prerequisites changed before completion');
        ...
        if (!remainingApprovals.length) {
          try { validateFinishVerification(store, done, verification, () => {
            ...
            if (openRaises(current).length) throw Object.assign(new Error('unanswered director forks — answer via chalk pending before finishing'), { needs: 'decision' });
            if (missingRequiredTest(store, current)) throw new Error('no required test remains in the change');
            const currentTasks = store.tasks();
            if (!depsSatisfied(current, currentTasks) || !depsSatisfied(selected, currentTasks)) throw new Error('task prerequisites changed during completion admission');
            if (needsReview(current) && !currentReview(store, current)) throw new Error(`review approval changed during admission — run chalk review ${current.id}`);
            const tracking = finishTrackingBlocker(store, current);
            if (tracking) throw new Error(tracking);
            const initial = finishContractCurrent(store, current, initialVerificationApproval, initialFinishPolicy, initialFinishContext);
            if (!initial.current) throw new Error(`initial verification inputs changed: ${initial.reason}`);
          }); }
          catch (error) { remainingApprovals.push(error.message); admissionNeeds = error.needs || 'review'; }
        }
      }
