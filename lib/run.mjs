...
import { finishTarget, formatFinishTarget, finishTrackingBlocker, finishPolicyIdentity, finishProbeScope, finishContractCurrent, validateFinishVerification } from './run-finish.mjs';
...
export function runDriver(store, { until = 'empty', max = 50, dryRun = false, finish, forceRerun = false, reviewRequiredNow = () => false, log = () => {} } = {}) {
  ...
  while (iterations < max) {
    ...
    if (!bi.skipped || !mut.skipped) {
      if (selected) {
        const initial = finishContractCurrent(store, store.task(t.id), initialVerificationApproval, initialFinishPolicy);
        if (!initial.current) {
    ...
    store.mutateTasks(tasks => {
      ...
      if (selected) {
        const initial = finishContractCurrent(store, done, initialVerificationApproval, initialFinishPolicy);
        ...
        if (!remainingApprovals.length) {
          try { validateFinishVerification(store, done, verification, () => {
            ...
            if (tracking) throw new Error(tracking);
            const initial = finishContractCurrent(store, current, initialVerificationApproval, initialFinishPolicy);
