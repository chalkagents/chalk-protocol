...
import { planApprovalRequired } from './planning.mjs';
  ...
  while (iterations < max) {
    ...
    if (reviewRequiredNow(store, store.task(t.id))) {
      ...
      if (!(r.status === 'ok' && r.verdict === 'pass')) {
        // Reviewer-induced: agent-owned work (fix the findings, re-review), not a human dependency (#46).
        blockTask(store, store.task(t.id), `review ${r.status === 'ok' ? r.verdict : 'error'}`, 'review');
        blocked.push(t.id);
        if (until === 'blocked') { stopped = 'blocked'; break; }
        continue;
      }
