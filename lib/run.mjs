...
import { now, runnableTasks, buildContext } from './store.mjs';
import { verify } from './verify.mjs';
import { runReview } from './review.mjs';

// The executor receives `chalk context` on stdin and edits the working tree. Its exit code is
// IGNORED — the verify gate, not the executor's self-report, decides success (preserves P4).
function runExecutor(store, task, cmd) {
  try {
    execSync(cmd, { cwd: store.root, input: buildContext(store, task), stdio: ['pipe', 'inherit', 'inherit'], timeout: 10 * 60 * 1000 });
...
export function runDriver(store, { until = 'empty', max = 50, dryRun = false, reviewRequiredNow = () => false, log = () => {} } = {}) {
  ...
  while (iterations < max) {
    ...
    if (!verify(store).green) {
      blockTask(store, store.task(t.id), 'verify RED after executor');
      blocked.push(t.id);
    ...
    if (reviewRequiredNow(store, store.task(t.id))) {
      ...
      if (r.status === 'ok') cur.reviews.push({ at: now(), by: 'adversary', verdict: r.verdict, findings: r.findings });
