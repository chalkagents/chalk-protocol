...
import { runAgent, usageForLedger } from './agent-runner.mjs';
...
function runExecutor(store, task, cmd) {
  ...
  const t0 = Date.now();
  const agentResult = runAgent('executor', { command: cmd, cwd: workdir(store, task), input: buildContext(store, task), output: { kind: 'text' } });
  store.logCost({ taskId: task.id, stage: 'work', agent: 'executor', ms: Date.now() - t0, ...usageForLedger(agentResult.usage) });
}
