...
// Build the argv for `opencode run`. Flags first (auto, model, attach), prompt always last & verbatim.
// `--auto` lets opencode EDIT files and run tools. The executor needs that; the JSON-contract roles
// (review/discovery/feedback) MUST NOT get it — a read-only reviewer that can edit the code it judges (or
// execute a prompt-injected diff's commands) is a P5-integrity hole. Callers pass `auto` explicitly; the
// default keeps `--auto` for back-compat.
export function buildRunArgs(prompt, opts = {}) {
  const autoArgs = opts.auto === false ? [] : ['--auto'];
