...
import { changedPaths, git, gitTry } from './git.mjs';
...
export function runBreakit(store, task, { cwd = workdir(store, task), strictExecution = false } = {}) {
  ...
  let stashed = false;
  try {
    git(cwd, ['stash', 'push', '--include-untracked', '--', ...impl]);
    stashed = true;
    ...
    for (const p of tests) {
      const result = launchShellCommand(withRunner(runner, tmpl.replace('{test}', p)), { cwd, stdio: 'pipe', env, timeout: 10 * 60 * 1000 });
      const unrunnable = result.status === 127 || result.error?.code === 'ENOENT' || result.error?.code === 'ETIMEDOUT' || result.signal
        || strictExecution && (result.status === 126 || result.error || !Number.isInteger(result.status));
      state.set(p, result.status === 0 && !result.error ? 'green' : unrunnable ? 'error' : 'red');
    }
    return {
      skipped: false,
    ...
    if (stashed) gitTry(cwd, ['stash', 'pop']);
  }
}
