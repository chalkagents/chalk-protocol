...
import { checkRoleCapabilities, decodeStructuredRole, roleContract } from './agent-contracts.mjs';
import { snapshotChanges, workspaceSnapshot } from './workspace-snapshot.mjs';
import { combineRoleInput, roleInstructions } from './role-instructions.mjs';
...
export function runAgent(role, request = {}) {
  ...
  const hasSeparateContext = request.context !== undefined;
  const context = hasSeparateContext ? request.context : request.input;
  const processInput = hasSeparateContext ? combineRoleInput(instructions, context) : (request.input ?? '');
  ...
  const output = contract?.output || request.output || { kind: 'text' };
  let status = processFailure.status;
  let structured = null;
  let text = String(unwrapped.text || '');
  if (output.kind === 'json') {
  ...
  if (changed.length) {
    diagnostics.push(diagnostic('error', 'read-only-mutation', `read-only ${role} changed: ${changed.join(', ')}`, false));
    status = 'failed';
    text = '';
    structured = null;
  }
