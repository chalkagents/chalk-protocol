...
const adapterCommand = (name) => `${JSON.stringify(process.execPath)} ${JSON.stringify(fileURLToPath(new URL(`../bin/adapters/${name}.mjs`, import.meta.url)))}`;
...
export function providerScaffold(name) {
    ...
    return {
      ...
      profiles: {
        claude: {
          ...
          identity: { displayName: 'Claude Code', independenceKey: 'claude:default' },
  ...
  if (name === 'opencode') {
    const legacyPath = fileURLToPath(new URL('../bin/adapters/opencode-exec.mjs', import.meta.url));
    return {
      legacy: { executor: `node ${legacyPath}` },
      profiles: {
        opencode: {
          adapter: 'opencode', command: adapterCommand('opencode'), model: null,
          identity: { displayName: 'OpenCode', independenceKey: 'opencode:default' },
          capabilities, options: {},
        },
      },
