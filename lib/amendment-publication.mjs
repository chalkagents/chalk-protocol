// An amended contract with an existing PR must carry its correction to the
// remote branch. Stage rank and an old green CI result cannot prove publication.
...
function candidate(store, task) {
  ...
  if (!task.branch) throw new Error('amended PR has no branch — restore its branch before chalk pr');
  const dirty = changedPaths(cwd).filter(p => !p.startsWith('.chalk/') || p.startsWith('.chalk/evidence/') || p.startsWith('.chalk/tests/'));
  if (dirty.length) throw new Error('amended PR has uncommitted code — run chalk commit, then chalk pr');
...
export function amendmentPublication(store, task) {
  ...
  try {
    ...
    return { ok: remote === head, required: true, head, detail: remote === head ? 'amended contract commit is on the PR branch' : 'amended PR branch is behind or different — run chalk pr before merge' };
  } catch (error) { return { ok: false, required: true, detail: `${error.message}; inspect the branch and retry chalk pr` }; }
}

export function publishAmendedPr(store, task) {
  const { cwd, head, ref } = candidate(store, task);
  ...
  const publication = amendmentPublication(store, task);
