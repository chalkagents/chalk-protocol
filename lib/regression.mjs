...
export function runAudit(store) {
  ...
  const reg = meta.protocol?.regression || {};
  const broken = brokenHeldOut(store.root, reg.tests);
  const size = codeSize(store.root);
  if (!reg.command) return { status: 'unconfigured', broken, size };
  let passed = true;
  try { execSync(withRunner(meta.protocol?.runner, reg.command), { cwd: store.root, stdio: 'ignore', timeout: 10 * 60 * 1000 }); }
  catch { passed = false; }
  return { status: 'ok', passed, green: passed && broken.length === 0, broken, size };
}
