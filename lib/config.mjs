...
export const REVIEWER_INDEPENDENCE_REMEDIATION = 'Set explicit, independently verified protocol.agents.profiles.<name>.identity.independenceKey values, or accept an unverified reviewer; changing providers or models alone does not prove independence';
...
function normalizeProfile(name, raw, source = 'configured') {
  ...
  return {
    ...
    capabilities: plainObject(profile.capabilities) ? profile.capabilities : null,
...
export function normGate(val) {
  ...
  if (val.timeoutMs !== undefined && (!Number.isSafeInteger(val.timeoutMs) || val.timeoutMs <= 0 || val.timeoutMs > 2147483647)) throw new Error('verification timeoutMs must be a positive integer no greater than 2147483647');
  return { cmd: String(val.cmd || ''), when, ...(val.timeoutMs !== undefined ? { timeoutMs: val.timeoutMs } : {}) };
}
