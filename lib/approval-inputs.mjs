...
import { canonicalCwd, digest, sourceIdentity, approvalSourceIdentity, refreshObservedSource, fileIdentity, directoryIdentity, verificationConfiguration, verificationIntegrityInputs, prepareVerificationOutputs, verificationDependencyContract } from './verification-record.mjs';
import { startVerificationMonitor } from './verification-command.mjs';

const contract = (task, tasks = [], includeDependencies = false) => {
  const dependencies = includeDependencies ? verificationDependencyContract(task, tasks) : [];
  return { id: task.id, title: task.title, revision: task.specRevision || 0, criteria: currentCriteria(task), tests: task.tests || [],
...
export function captureApproval(store, kind, task, { source, cwd = kind === 'audit' ? store.root : workdir(store, task) } = {}) {
  try {
    ...
    const tasks = typeof store.tasks === 'function' ? store.tasks() : task ? [task] : [];
    const dependencyAware = ['audit', 'review', 'verification'].includes(kind);
    const spec = { project: store.meta().project?.description || '', spec: store.spec?.() || '',
      contract: kind === 'audit' ? resolvedTaskHistory(store).map(item => contract(item, tasks, dependencyAware)).sort((a, b) => a.id.localeCompare(b.id)) : contract(task, tasks, dependencyAware) };
