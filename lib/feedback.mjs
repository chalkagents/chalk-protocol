...
export function runFeedback(store, signals) {
  ...
  let raw = '';
  const t0 = Date.now();
  try { raw = execSync(withRunner(store.protocol().runner, cmd), { cwd: store.root, input: signals, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 10 * 60 * 1000 }); }
  catch (e) { raw = `${e.stdout || ''}${e.stderr || ''}`; }
  try { store.logCost({ stage: 'feedback', agent: 'feedback', ms: Date.now() - t0 }); } catch { /* ledger best-effort */ }
  const o = parseLastJson(raw, (x) => Array.isArray(x.issues));
  if (!o) return { status: 'error', raw: (raw || '').slice(-400) };
