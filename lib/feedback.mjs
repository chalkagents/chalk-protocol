...
export function collectSignals(store, { input = '' } = {}) {
  ...
  const parts = [];
  for (const p of files) { try { parts.push(`### ${basename(p)}\n${readFileSync(p, 'utf8').trim()}`); } catch { /* unreadable */ } }
...
export function runFeedback(store, signals) {
  const profile = resolveAgentRole(store.protocol(), 'feedback');
  if (!profile?.command) return { status: 'unconfigured' };
  const agentResult = runAgent('feedback', {
    profile, cwd: store.root, context: signals, stderr: 'capture',
