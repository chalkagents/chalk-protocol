...
function captureDiff(root, base) {
  ...
  for (const args of cmds) {
    try {
      const out = git(root, ['-c', 'diff.autoRefreshIndex=false', '--no-optional-locks', ...args, ...diffPathspec]);
      if (out.trim()) {
        let stat = '';
        try { stat = git(root, ['-c', 'diff.autoRefreshIndex=false', '--no-optional-locks', args[0], '--stat', ...args.slice(1), ...diffPathspec]); } catch { /* stat is best-effort */ }
...
export function runReview(store, task) {
  ...
  return { status: 'ok', ...parsed, raw: text, approval };
