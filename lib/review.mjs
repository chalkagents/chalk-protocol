...
// Reviewer retries are deliberately narrower than the adapter's open-ended diagnostic namespace.
// These are failures that can plausibly clear when the same read-only request is repeated once.
// Invalid schemas, permission/capability failures, input races and workspace mutations are terminal.
export const TRANSIENT_REVIEW_DIAGNOSTICS = Object.freeze([
  'timeout',
  'provider-timeout',
  'transport-error',
  'adapter-transport',
...
const transientReviewDiagnostics = new Set(TRANSIENT_REVIEW_DIAGNOSTICS);
export const REVIEW_TRANSIENT_EXIT = 4;

export function reviewFailureIsTransient(review) {
  if (review?.status !== 'error') return false;
  const errors = (review.diagnostics || []).filter((item) => !item?.level || item.level === 'error');
  return errors.length > 0 && errors.every((item) => item.retryable === true && transientReviewDiagnostics.has(item.code));
}

export function runReviewWithRetry(attempt, { retry = true, onRetry = () => {} } = {}) {
  let result = attempt();
  if (!retry || !reviewFailureIsTransient(result)) return result;
  onRetry(result);
  result = attempt();
  return result;
}
...
function formatUsage(usage) {
  ...
  const fields = [
    ...
    ['costUsd', 'USD'], ['turns', 'turns'],
  ];
  return fields.filter(([key]) => usage[key] !== undefined).map(([key, label]) => `${usage[key]} ${label}`).join(' · ');
}

export function formatReviewFailure(review) {
  const invocation = review?.invocation || {};
  const heading = `reviewer ${invocation.status || 'failed'}${Number.isFinite(invocation.durationMs) ? ` after ${invocation.durationMs} ms` : ''}`;
  const lines = [heading];
  for (const item of review?.diagnostics || []) {
    lines.push(`[${item.code || 'diagnostic'}] ${item.message || '(no message)'}${item.retryable === true ? ' (transient)' : ''}`);
  }
  const usage = formatUsage(invocation.usage);
  if (usage) lines.push(`usage: ${usage}`);
  if (review?.raw) lines.push(`raw tail: ${String(review.raw).slice(-600)}`);
  lines.push('No verdict was accepted.');
  return lines.join('\n  ');
}

export function reviewResultFromAgent(agentResult, { freshness = { current: true }, durationMs = 0, approval, inputs } = {}) {
  const text = String(agentResult?.text || '');
  const diagnostics = Array.isArray(agentResult?.diagnostics) ? agentResult.diagnostics : [];
  const invocation = {
  ...
  const readOnlyFailure = diagnostics.some((item) => item.code === 'read-only-mutation');
  // Raw-command compatibility historically accepts a complete verdict printed before a non-zero
  // exit (some reviewer commands use that exit to signal BLOCK). No other failed/timeout response
  // may smuggle a verdict through its structured field.
  const legacyNonzeroVerdict = agentResult?.status === 'failed'
    && diagnostics.some((item) => item.code === 'nonzero-exit')
    && diagnostics.every((item) => ['nonzero-exit', 'stderr'].includes(item.code));
  if (!freshness.current && !readOnlyFailure) return {
    status: 'error', raw: freshness.reason,
    diagnostics: [...diagnostics, { code: 'approval-inputs', message: freshness.reason, retryable: false }],
    invocation,
  };
  if (!parsed || (agentResult?.status !== 'ok' && !legacyNonzeroVerdict)) return { status: 'error', raw: text.slice(-600), diagnostics, invocation };
  ...
  return { status: 'ok', ...parsed, raw: text, approval, inputs: manifest, invocation };
}
...
export function runReview(store, task, { onInputs = () => {} } = {}) {
  ...
  return reviewResultFromAgent(agentResult, { freshness, durationMs, approval, inputs });
