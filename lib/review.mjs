...
// Render the decision digest (#192) as plain lines (no ANSI — the caller adds colour). Each line names
// the choice with its blast-radius and reversibility so a director can triage at a glance. Tolerant of
// a reviewer that omits fields. Empty in → empty out, so a change with no judgment calls prints nothing.
export function formatDecisionDigest(decisions) {
  if (!Array.isArray(decisions)) return [];
  return decisions
    .filter((d) => d && (d.choice || d.rationale))
    .map((d) => {
