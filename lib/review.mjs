...
function captureDiff(root, base) {
  ...
  for (const cmd of cmds) {
    try {
      ...
      if (out.trim()) {
        let stat = '';
        try { stat = execSync(`${cmd} --stat`, { cwd: root, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }); } catch { /* stat is best-effort */ }
...
export function runReview(store, task) {
  ...
  try {
    ...
    raw = execSync(withJsonOutput(withRunner(meta.protocol?.runner, cmd)), { cwd: wd, input: prompt, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 10 * 60 * 1000 });
  } catch (e) {
