import { checkApproval } from './approval-inputs.mjs';
...
export function planApprovalRequired(store, task) {
  return !!store.protocol().plan?.required && !checkApproval(store, 'plan', task.planApproved, task).current;
}

// The alignment gate (the director checkpoint): when director mode requires it, `work` won't build
// until a human has ACCEPTED the acceptance criteria as the definition of done. Where the plan gate
// asks "is the approach right?", this asks "is *done* framed right?" — the empty-middle risk in #160,
// where an autonomous run builds everything and ends up misaligned. Opt-in via protocol.director.required
// (default off), so existing/unattended flows are unaffected. Mirrors planApprovalRequired.
export function criteriaAcceptedRequired(store, task) {
  return !!store.protocol().director?.required && !checkApproval(store, 'alignment', task.criteriaAccepted, task).current;
}

// Recheck at admission, not just before an executor starts: an amendment can
// invalidate these approvals while work is running.
export function completionApprovalBlockers(store, task) {
  const reasons = [];
  if (planApprovalRequired(store, task)) reasons.push(`plan not approved — run chalk approve-plan ${task.id}`);
  if (criteriaAcceptedRequired(store, task)) reasons.push(`criteria not accepted — run chalk align ${task.id}`);
  return reasons;
}
