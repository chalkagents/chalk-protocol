...
function pathCandidates(command, env, platform) {
  ...
  const extensions = platform === 'win32' && !extension
    // Try the literal file first so portable, extensionless Node shebang fixtures work on
    // Windows too; native commands still fall through to the normal PATHEXT candidates.
