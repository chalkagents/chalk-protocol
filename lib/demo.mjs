...
export function runDemo({ keep = false, log = console.log } = {}) {
  ...
  try {
    ...
    step(['init', '--name', 'Widget App', '--goal', 'A little widget app', '--bare']); // demo wires its own verify next
    ...
    step(['spec', id, '--test', 'widgets.test.mjs', '--why', 'demo: attach the implemented acceptance test']);
    step(['plan', id]);
    step(['approve-plan', id, '--force', '--why', 'demo: regenerated plan covers the new test lock; persistence deferred']);
