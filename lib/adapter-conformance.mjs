...
function requestFor(fixture, workingDirectory, live, options) {
  ...
  return {
    ...
    // Ordinary probes include process startup, which can exceed two seconds
    // under concurrent verification. Keep the adversarial timeout probe tight.
    timeoutMs: fixture === 'timeout' ? 40 : 5_000,
    workingDirectory,
...
function invoke(command, request) {
  ...
  const result = launchCommandString(command, {
  ...
  const normalized = refuseReadOnlyMutation(request.role, {
    status: validEnvelope ? response.status : 'failed',
  ...
  return { result, response, parseError, validEnvelope, changed, normalized };
}
...
function assess(fixture, invocation, request) {
  const { result, response, parseError, validEnvelope, changed, normalized } = invocation;
  ...
  if (fixture === 'multiline') { ok = normal() && response.text === request.context; detail = 'multiline context preserved verbatim'; }
    ...
    const refusal = normalized.diagnostics?.find((item) => item.code === 'read-only-mutation'
      && changed.every((path) => String(item.message || '').includes(path)));
    ok = changed.length > 0 && normalized.status === 'failed'
      && changed.every((path) => String(refusal?.message || '').includes(path));
    detail = ok
      ? `production result failed with read-only-mutation: ${changed.join(', ')}`
      : 'mutation fixture did not prove production refusal with changed paths';
  }
  const assessed = ok ? pass(fixture, detail) : fail(fixture, `${detail}; exit=${String(result.status)}${result.error?.code ? `; error=${result.error.code}` : ''}${result.signal ? `; signal=${result.signal}` : ''}${parseError ? `; ${parseError}` : ''}`);
  return fixture === 'mutation' && normal() ? { ...assessed, adapterViolation: true } : assessed;
}
...
export function runAdapterConformance({ command, adapter = 'external', live = false, options = {} } = {}) {
  ...
  const adapterViolations = results.filter((item) => item.adapterViolation).map((item) => item.name);
  return {
    protocolVersion: ADAPTER_PROTOCOL_VERSION,
    adapter,
    ...
    ok: results.every((item) => item.status === 'pass') && adapterViolations.length === 0,
    adapterViolations,
    results,
  };
}
...
export function renderConformanceReport(report) {
  const lines = [
    ...
    '',
    ...report.results.map((item) => `${item.status === 'pass' ? 'PASS' : 'FAIL'}  ${item.name} [${item.protocolVersion}] — ${item.detail}${item.adapterViolation ? ' · ADAPTER VIOLATION: returned ok after mutation' : ''}`),
    '',
    `${report.passed}/${report.total} passed · overall ${report.ok ? 'PASS' : 'FAIL'}`,
  ];
  return lines.join('\n');
}
