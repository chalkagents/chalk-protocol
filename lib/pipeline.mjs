...
import { REVIEW_TRANSIENT_EXIT } from './review.mjs';
...
export function runPipeline(store, cliPath, { max = 20, dryRun = false, only = null, stopBefore = null, log = () => {} } = {}) {
  ...
  const planner = resolveAgentRole(proto, 'planner');
  const reviewer = resolveAgentRole(proto, 'reviewer');
  ...
  for (const t of queue) {
    ...
    for (const cmd of stages) {
      if (cmd === 'plan' && !planner?.command) { log('  plan: skipped (no planner)'); continue; }
      if (cmd === 'review' && !reviewer?.command) { log('  review: skipped (no reviewer)'); continue; }
      ...
      } else if (cmd === 'review' && r.status === REVIEW_TRANSIENT_EXIT) {
