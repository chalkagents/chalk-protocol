...
import { brokenHeldOut } from './regression.mjs';
...
export function auditApprovalCurrent(store, audit = store.protocol().regression?.lastAudit) {
  if (!audit?.green || !checkApproval(store, 'audit', audit).current) return false;
  try { return brokenHeldOut(store.root, store.protocol().regression?.tests || []).length === 0; }
  catch { return false; }
}
