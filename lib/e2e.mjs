...
export function runSpecs(store, cwd, specPaths, { execute, protocol = store.protocol() } = {}) {
  ...
  for (const path of specPaths.filter((p) => isSpec(p, e2e.specPattern))) {
    ...
    try {
      ...
      if (execute) { execution = execute(command); status = execution.status === 'pass' ? 'passed' : 'failed'; }
      else { execSync(command, { cwd, stdio: 'inherit', timeout: 10 * 60 * 1000 }); status = 'passed'; }
