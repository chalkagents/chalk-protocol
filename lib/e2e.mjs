...
import { DEFAULT_VERIFICATION_TIMEOUT_MS } from './verification-command.mjs';

// Which locked test paths are browser SPECS (run via the BYO e2e command), not code tests. The
// convention is a suffix; teams with a different one set `protocol.e2e.specPattern` (#83) — a
// suffix, a comma-separated list, or an array. A leading `*` (glob-ish) is stripped so
// `*.e2e.yaml` matches by suffix. Empty/unset → the historical `.test.yaml`.
export function specMatcher(pattern) {
  const raw = Array.isArray(pattern) ? pattern : (pattern ? String(pattern).split(',') : []);
  const suffixes = raw.map((s) => String(s).trim().replace(/^\*/, '')).filter(Boolean);
  const list = suffixes.length ? suffixes : ['.test.yaml'];
  return (p) => typeof p === 'string' && list.some((sfx) => p.endsWith(sfx));
}
export const isSpec = (p, pattern) => specMatcher(pattern)(p);
...
export function runSpecs(store, cwd, specPaths, { execute, protocol = store.protocol(), timeoutMs = DEFAULT_VERIFICATION_TIMEOUT_MS } = {}) {
  const e2e = protocol.e2e || {};
  const runner = protocol.runner;
  const out = [];
  if (!e2e.command) return out;
  for (const path of specPaths.filter((p) => isSpec(p, e2e.specPattern))) {
    ...
    let execution;
    try {
      const command = `${withRunner(runner, e2e.command)} --spec ${path} --out ${runDir}${base}`;
      if (execute) { execution = execute(command, { timeoutMs }); status = execution.status === 'pass' ? 'passed' : 'failed'; }
      else { execSync(command, { cwd, stdio: 'inherit', timeout: timeoutMs }); status = 'passed'; }
    } catch (error) { if (execute) throw error; status = 'failed'; }
    // Prefer the runner's own verdict from run.json if it wrote one.
    try { const r = JSON.parse(readFileSync(join(cwd, runDir, 'run.json'), 'utf8')); if (r.status && (!execution || execution.status === 'pass')) status = r.status; } catch { /* keep exit-code verdict */ }
    out.push({ path, specId, runId, status, runDir, ...(execution ? { execution } : {}) });
  }
  return out;
}
