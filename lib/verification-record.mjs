...
export async function fileIdentityAsync(path, stat) {
  stat ||= await fsAsync.lstat(path, { bigint: true });
...
export async function assertStorageAsync(dir, identity) {
  const stat = await fsAsync.lstat(dir, { bigint: true });
  const { inode, device } = exactStatIdentity(stat);
  if (!stat.isDirectory() || !identity || digest({ path: await fsAsync.realpath(dir), inode, device }) !== digest(identity)) {
