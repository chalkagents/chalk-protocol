...
// Fresh endpoint manifest for approval admission. This is deliberately NOT an
// observation baseline: execution still needs sourceIdentity's policy, index and
// namespace authorities. Re-enumerate inputs on every call; never cache a PASS.
export function approvalSourceIdentity(cwd, proto = {}) {
  return { ...fingerprintSource(cwd, proto, false, true), manifest: 'endpoint-only' };
}
...
function fingerprintSource(cwd, proto, filesystemOnly = false, manifestOnly = false) {
  ...
  try {
    ...
    if (!filesystemOnly && git.status === 0 && git.stdout.trim() === 'true') {
      method = 'git';
      if (!manifestOnly) Object.assign(namespaceInputs, sourceDirectories(root, hidden, method));
      ...
      for (const path of policy) {
        ...
        // Git legitimately resolves policy through symlinked ancestors (macOS
        // /etc is one example). Observe each selector as well as policy bytes,
        // so redirecting and restoring the ancestor cannot hide a policy change.
        for (let dir = dirname(path); ; dir = dirname(dir)) {
          try {
            if (lstatSync(dir).isSymbolicLink()) membershipInputs[dir] = { kind: 'locator', identity: identifyMembership(dir, 'locator'), resolved: canonicalCwd(dir) };
      ...
      // Gitlink fingerprints include nested policy identity. Use the complete
      // existing scanner for exact compatibility with execution receipts.
      if (manifestOnly && gitlinks.size) return fingerprintSource(cwd, proto, filesystemOnly);
    } else {
...
export function refreshObservedSource(cwd, before) {
  ...
  try {
    ...
    for (const [path, input] of Object.entries(before.membershipInputs)) {
      const actual = canonicalCwd(path);
      if (actual !== path && before.membershipInputs[actual] === undefined && input.resolved !== actual) throw new Error('membership path redirected during verification');
      membershipInputs[path] = { ...input, identity: membershipIdentity(path, input.kind) };
      if (input.kind === 'shared') sharedMetadata[path] = fileIdentity(path);
    }
...
export function verificationDependencyContract(task, tasks = []) {
  return (task.after || []).map(ref => {
    const exact = tasks.filter(candidate => candidate.id === ref);
    const matches = exact.length ? exact : tasks.filter(candidate => candidate.id.startsWith(ref));
    if (matches.length !== 1) return { ref, status: matches.length ? 'ambiguous' : 'missing', matches: matches.map(candidate => candidate.id).sort() };
    const dependency = matches[0], revision = dependency.specRevision || 0;
    return { ref, status: 'resolved', id: dependency.id, state: dependency.state,
      revision, completedRevision: dependency.completedSpecRevision ?? null,
      completionInvalidated: dependency.completionInvalidated ?? null,
      complete: dependency.state === 'done' && !dependency.completionInvalidated &&
        (dependency.completedSpecRevision === undefined || dependency.completedSpecRevision === revision) };
  });
}

export const verificationTaskContract = (task, tasks = []) => {
  const dependencies = verificationDependencyContract(task, tasks);
  return { id: task.id, specRevision: task.specRevision || 0, acceptanceCriteria: task.acceptanceCriteria || [], tests: task.tests || [],
    ...(dependencies.length ? { dependencies } : {}) };
};

// The same visible integrity scope feeds verification and audit approval binding.
export function verificationIntegrityInputs(store, cwd, proto = store.protocol()) {
  return store.tasks().filter(t => t.state === 'in-progress' || (proto.integrity === 'all-locks' && t.state === 'done')).map(t => {
...
// Prepare protocol-owned outputs before any enclosing observer binds namespaces.
// Audit calls this before its outer observation; standalone verify uses it too.
export function prepareVerificationOutputs(store, cwd, proto) {
  if (proto.e2e?.command) {
...
export function verificationInputs(store, cwd, observedSource) {
  ...
  if (!observedSource) prepareVerificationOutputs(store, cwd, proto);
  const config = verificationConfiguration(proto);
  const allTasks = store.tasks();
  const tasks = allTasks.filter(t => t.state === 'in-progress' && canonicalCwd(workdir(store, t)) === canonicalCwd(cwd))
    .map(task => verificationTaskContract(task, allTasks)).sort((a, b) => a.id.localeCompare(b.id));
  const integrityInputs = verificationIntegrityInputs(store, cwd, proto);
  ...
  const authorityFiles = Object.fromEntries([store.p.tasks, store.p.chalk, store.p.spec].filter(Boolean).map(path => {
    try { return [path, fileIdentity(path)]; }
...
export function createVerificationRecord(store, cwd, mode) {
  ...
  const record = { version: 1, id, cwd: canonicalCwd(cwd), mode, startedAt: new Date().toISOString(), status: 'running',
    runtime: { node: process.version, platform: process.platform, arch: process.arch }, storage: evidence.storage, toolchain: [] };
...
export function saveVerificationRecord(evidence, record) {
  ...
  atomicWrite(evidence.path, bytes, evidence.storage || storageIdentity(dirname(evidence.path)));
  return digest(bytes);
}
