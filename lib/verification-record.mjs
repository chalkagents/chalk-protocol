...
const exactStatValue = (stat, nanoseconds, milliseconds) => String(stat[nanoseconds] ?? stat[milliseconds]);
export const exactStatIdentity = stat => ({ inode: String(stat.ino), device: String(stat.dev) });
export const exactStatMetadata = stat => ({
  ...exactStatIdentity(stat),
  modified: exactStatValue(stat, 'mtimeNs', 'mtimeMs'),
  changed: exactStatValue(stat, 'ctimeNs', 'ctimeMs'),
});
const executableBits = stat => Number(typeof stat.mode === 'bigint' ? stat.mode & 0o111n : stat.mode & 0o111);
...
export const isProtectedVerificationPath = (path, hidden) => {
  const portable = slash(path);
  return path === hidden || path.startsWith(hidden + sep) ||
    portable.endsWith('/.chalk/held-out') || portable.includes('/.chalk/held-out/');
};
...
const decodeGitPathBytes = (value, label) => {
  try { return new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(value); }
  catch { throw new Error(`cannot decode ${label} as UTF-8`); }
};
const GIT_PATH_ENVIRONMENT = new Set([
...
const assertGitPathEnvironment = () => {
  for (const [name, value] of Object.entries(process.env)) {
    const slot = /^GIT_CONFIG_VALUE_(\d+)$/.exec(name)?.[1];
    const key = slot === undefined ? '' : String(process.env[`GIT_CONFIG_KEY_${slot}`] || '').toLowerCase();
    const configPath = key === 'core.excludesfile' || /^(include|includeif[.].*)[.]path$/.test(key);
    if ((GIT_PATH_ENVIRONMENT.has(name) || configPath) && value?.includes('\uFFFD')) {
      throw new Error(`cannot safely use replacement character in Git pathname environment: ${name}`);
    }
  }
};
...
function fingerprintSource(cwd, proto, filesystemOnly = false) {
  ...
  const identifyMembership = (path, kind) => {
    ...
    if (isProtectedVerificationPath(parent, hiddenRoot)) throw new Error('held-out path cannot be a membership namespace');
  ...
  try {
    ...
    if (!filesystemOnly) assertGitPathEnvironment();
    ...
    if (!filesystemOnly && git.status === 0 && git.stdout.trim() === 'true') {
      ...
      const listed = spawnSync('git', ['ls-files', '--cached', '--others', '--exclude-standard', '-z', ...pathspec], { cwd: root, maxBuffer: 64 * 1024 * 1024 });
      if (listed.status !== 0 || listed.error) throw new Error('cannot enumerate git inputs');
      paths = decodeGitPathBytes(listed.stdout, 'Git input paths').split('\0').filter(Boolean);
      ...
      for (const args of [['--git-path', 'index'], ['--shared-index-path']]) {
        const index = spawnSync('git', ['rev-parse', ...args], { cwd: root });
        if (index.status !== 0) throw new Error('cannot locate Git input membership');
        const indexPath = gitPathLine(decodeGitPathBytes(index.stdout, 'Git input membership path'));
        if (!indexPath) continue;
        const path = resolve(root, indexPath), actual = canonicalCwd(path), hiddenRoot = canonicalCwd(resolve(root, raw));
        if (isProtectedVerificationPath(actual, hiddenRoot)) throw new Error('held-out path cannot be an index input');
      ...
      const origins = spawnSync('git', ['config', '--show-origin', '--name-only', '--null', '--list'], { cwd: root });
      const info = spawnSync('git', ['rev-parse', '--git-path', 'info/exclude'], { cwd: root });
      const global = spawnSync('git', ['config', '--null', '--path', '--get', 'core.excludesfile'], { cwd: root });
      if (origins.status !== 0 || info.status !== 0 || ![0, 1].includes(global.status)) throw new Error('cannot identify Git ignore policy');
      const originsOutput = decodeGitPathBytes(origins.stdout, 'Git configuration origins');
      const infoPath = gitPathLine(decodeGitPathBytes(info.stdout, 'Git exclude path'));
      const globalOutput = global.status === 0 ? decodeGitPathBytes(global.stdout, 'Git ignore-policy path') : '';
      if (global.status === 0 && !globalOutput.endsWith('\0')) throw new Error('cannot decode Git ignore-policy path');
      const externalIgnore = global.status === 0 ? resolve(root, globalOutput.slice(0, -1)) : join(process.env.XDG_CONFIG_HOME || join(homedir(), '.config'), 'git/ignore');
      const policy = new Set([resolve(root, infoPath), resolve(root, '.gitignore'), externalIgnore]);
      const explicitAuthorities = new Set([resolve(root, infoPath), externalIgnore]);
      ...
      for (const name of ['GIT_CONFIG_SYSTEM', 'GIT_CONFIG_GLOBAL']) {
        const candidates = spawnSync('git', ['var', name], { cwd: root });
        if (candidates.status !== 0) throw new Error(`cannot locate ${name}; use Git with configuration-path discovery support`);
        const value = gitPathLine(decodeGitPathBytes(candidates.stdout, `${name} path`));
        let paths;
      ...
      for (const name of ['config', 'config.worktree', 'HEAD', 'commondir']) {
        const candidate = spawnSync('git', ['rev-parse', '--git-path', name], { cwd: root });
        if (candidate.status !== 0) throw new Error('cannot locate repository configuration');
        addAuthority(resolve(root, gitPathLine(decodeGitPathBytes(candidate.stdout, 'repository configuration path'))));
      }
      const includes = spawnSync('git', ['config', '--path', '--show-origin', '--null', '--get-regexp', '^(include|includeif[.].*)[.]path$'], { cwd: root });
      if (![0, 1].includes(includes.status)) throw new Error('cannot locate included configuration');
      const included = decodeGitPathBytes(includes.stdout, 'included Git configuration paths').split('\0');
      ...
      const top = spawnSync('git', ['rev-parse', '--show-toplevel'], { cwd: root });
      if (top.status !== 0) throw new Error('cannot locate ancestor ignore policies');
      const gitRoot = canonicalCwd(gitPathLine(decodeGitPathBytes(top.stdout, 'Git worktree path')));
      ...
      for (let dir = root; ; dir = dirname(dir)) {
        policy.add(join(dir, '.gitignore')); selectors.add(join(dir, '.git'));
        if (sameDirectory(dir, gitRoot)) break;
        if (dirname(dir) === dir) throw new Error('Git worktree root is not an ancestor of the verification root');
      }
      for (const arg of ['--absolute-git-dir', '--git-common-dir']) {
        const selected = spawnSync('git', ['rev-parse', arg], { cwd: root });
        if (selected.status !== 0) throw new Error('cannot locate repository selector');
        selectors.add(resolve(root, gitPathLine(decodeGitPathBytes(selected.stdout, 'Git repository selector path'))));
      }
      for (const path of selectors) {
        const actual = canonicalCwd(path), hiddenRoot = canonicalCwd(resolve(root, raw));
        if (isProtectedVerificationPath(actual, hiddenRoot)) throw new Error('held-out path cannot be a repository selector');
        for (const p of new Set([path, actual])) membershipInputs[p] = { kind: 'locator', identity: identifyMembership(p, 'locator') };
      }
      const fields = originsOutput.split('\0');
      ...
      for (const path of policy) {
        const actual = canonicalCwd(path), hiddenRoot = canonicalCwd(resolve(root, raw));
        if (isProtectedVerificationPath(actual, hiddenRoot)) throw new Error('held-out path cannot be an ignore-policy input');
        const parent = namespaceParent(path);
        if (isProtectedVerificationPath(parent, hiddenRoot)) throw new Error('held-out path cannot be an authority namespace');
        ...
        try { policyInputs[path] = fileIdentity(path); if (actual !== path) policyInputs[actual] = fileIdentity(actual); }
          ...
          if (basename(path) === '.gitignore' && !explicitAuthorities.has(path) &&
              withinDirectory(parent, gitRoot) && sameDirectory(parent, dirname(path))) {
      ...
      const staged = spawnSync('git', ['ls-files', '--stage', '-z', ...pathspec], { cwd: root, maxBuffer: 64 * 1024 * 1024 });
      if (staged.status !== 0 || staged.error) throw new Error('cannot enumerate gitlink inputs');
      for (const entry of decodeGitPathBytes(staged.stdout, 'Git index paths').split('\0')) {
    ...
    for (const p of [...new Set(paths)].sort()) {
      ...
      try {
        ...
        if (stat.isSymbolicLink()) {
          ...
          const top = spawnSync('git', ['rev-parse', '--show-toplevel'], { cwd: abs });
          const initialized = top.status === 0 && canonicalCwd(gitPathLine(decodeGitPathBytes(top.stdout, 'submodule worktree path'))) === canonicalCwd(abs);
...
export function refreshObservedSource(cwd, before) {
  ...
  try {
    ...
    for (const [path, identity] of Object.entries(before.files)) {
      ...
      try {
        const abs = join(root, path), stat = lstatSync(abs, { bigint: true });
...
export function fileIdentity(path, stat = lstatSync(path, { bigint: true })) {
  return digest({ content: digest(readFileSync(path)), executable: executableBits(stat), ...exactStatMetadata(stat) });
}

export function membershipIdentity(path, kind) {
  try {
    const stat = lstatSync(path, { bigint: true });
    if (kind === 'shared') return digest({ content: digest(readFileSync(path)), inode: String(stat.ino) });
...
export async function fileIdentityAsync(path, stat) {
  // Node's synchronous and promise stat bindings can expose different Windows file IDs
  // for the same path. All cross-boundary comparisons use the synchronous binding for
  // metadata while retaining asynchronous, bounded content reads below.
  stat ||= lstatSync(path, { bigint: true });
  return digest({ content: await contentIdentityAsync(path), executable: executableBits(stat), ...exactStatMetadata(stat) });
}

export async function membershipIdentityAsync(path, kind) {
  try {
    const stat = lstatSync(path, { bigint: true });
    if (kind === 'shared') return digest({ content: await contentIdentityAsync(path), inode: String(stat.ino) });
    if (kind === 'locator' && (stat.isDirectory() || stat.isSymbolicLink())) {
      return digest({ directory: stat.isDirectory(), link: stat.isSymbolicLink() ? readlinkSync(path) : null,
        ...exactStatMetadata(stat) });
    }
...
export async function canonicalCwdAsync(path) {
  // Match canonicalCwd exactly across the verification boundary. Windows may expose
  // different aliases through synchronous and promise-based filesystem bindings.
  return canonicalCwd(path);
}

export async function assertStorageAsync(dir, identity) {
  const stat = lstatSync(dir, { bigint: true });
  const { inode, device } = exactStatIdentity(stat);
  if (!stat.isDirectory() || !identity || digest({ path: realpathSync(dir), inode, device }) !== digest(identity)) {
...
export function storageIdentity(dir) {
  ...
  const { inode, device } = exactStatIdentity(stat);
  return { path: realpathSync(dir), inode, device };
}
...
export function verificationInputs(store, cwd, observedSource) {
  ...
  if (!observedSource && proto.e2e?.command) {
    ...
    if ([output, parent].some(path => isProtectedVerificationPath(path, hidden))) throw new Error('held-out path cannot be browser output');
    mkdirSync(output, { recursive: true });
  }
  const config = { verify: proto.verify || {}, runner: proto.runner || '', e2e: proto.e2e || {}, integrity: proto.integrity || '', regression: { dir: proto.regression?.dir || '.chalk/held-out' } };
  const tasks = store.tasks().filter(t => t.state === 'in-progress' && canonicalCwd(workdir(store, t)) === canonicalCwd(cwd))
    .map(t => ({ id: t.id, acceptanceCriteria: t.acceptanceCriteria || [], tests: t.tests || [] })).sort((a, b) => a.id.localeCompare(b.id));
  const integrityInputs = store.tasks().filter(t => t.state === 'in-progress' || (proto.integrity === 'all-locks' && t.state === 'done')).map(t => {
    ...
    const tests = (t.tests || []).map(lock => {
      ...
      if (isProtectedVerificationPath(actualPath, hidden)) throw new Error('held-out path cannot be a visible integrity input');
      try {
