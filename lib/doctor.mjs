...
import { reviewCadences, normGate, resolveAgentConfiguration, compareReviewerIndependence } from './config.mjs';
...
export function doctorAgentSummary(protocol = {}) {
  ...
  return {
    ...
    independence: {
      ...
      nextAction: independence.status === 'distinct' ? null
        : 'chalk connect --replace --builder <adapter> --reviewer <different-adapter>',
    },
...
export function runDoctor(store) {
  ...
  if (cadences.length && executorProfile?.command && reviewerProfile?.command) {
    ...
    if (independence.status === 'same') add('gates', 'warn', 'P5 reviewer has the same explicit independence identity as the executor — fix: chalk connect --replace --builder <adapter> --reviewer <different-adapter>');
    else if (independence.status === 'distinct') add('gates', 'ok', 'P5 reviewer independence verified by distinct explicit identity keys');
    else add('gates', 'warn', 'P5 reviewer independence unverified — fix: chalk connect --replace --builder <adapter> --reviewer <different-adapter>');
  }
