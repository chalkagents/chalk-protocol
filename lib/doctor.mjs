...
import { reviewCadences, sameModelFamily, normGate, resolveAgentConfiguration, compareReviewerIndependence } from './config.mjs';
...
export function doctorAgentSummary(protocol = {}) {
  ...
  return { version: resolved.version, roles, problems: resolved.problems };
}
...
export function runDoctor(store) {
  ...
  if (cadences.length && !reviewerProfile?.command) add('gates', 'warn', `review cadence ${JSON.stringify(cadences)} set but no reviewer role is bound`);
  ...
  if (cadences.length && executorProfile?.command && reviewerProfile?.command) {
    ...
    if (independence.status === 'same') add('gates', 'warn', 'P5 reviewer has the same explicit independence identity as the executor — bind a profile with a distinct identity.independenceKey');
    else if (independence.status === 'distinct') add('gates', 'ok', 'P5 reviewer independence verified by distinct explicit identity keys');
    else {
      add('gates', 'warn', 'P5 reviewer independence unverified — set explicit identity.independenceKey values on the executor and reviewer profiles');
      // Legacy-only compatibility hint for existing installations. It is non-authoritative: the
      // explicit status remains "unverified" until profiles provide keys.
      if (executorProfile.source === 'legacy' && reviewerProfile.source === 'legacy'
        && sameModelFamily(executorProfile.command, reviewerProfile.command))
        add('gates', 'warn', 'legacy command heuristic suggests the reviewer uses the same model as the executor; migrate to explicit profile identities to verify independence');
    }
  }
