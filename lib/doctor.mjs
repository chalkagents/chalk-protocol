...
import { reviewCadences, normGate, resolveAgentConfiguration, compareReviewerIndependence, REVIEWER_INDEPENDENCE_REMEDIATION } from './config.mjs';
import { checkRoleCapabilities } from './agent-contracts.mjs';
import { commandWords, launchCommand } from './process.mjs';

// Probe through Node's platform-native executable lookup instead of a POSIX shell built-in.
// A non-zero exit still means the executable was found; only a spawn error means lookup failed.
export const commandOnPath = (cmd) => {
  const [bin, ...args] = commandWords(cmd);
  if (!bin) return false;
  return !launchCommand(bin, [...args, '--version'], { stdio: 'ignore', windowsHide: true }).error;
};
const ghAuthed = (ghCmd) => {
  try {
    const [bin, ...args] = commandWords(ghCmd || 'gh');
    const result = launchCommand(bin, [...args, 'auth', 'status'], { stdio: 'ignore', timeout: 30000 });
...
// Credential-safe structured agent report for `chalk doctor --json`. Connection commands and
// profile options are intentionally omitted; model/display/independence values are diagnostic only.
export function doctorAgentSummary(protocol = {}) {
  ...
  for (const [role, profile] of Object.entries(resolved.roles)) {
    ...
    roles[role] = {
      ...
      capabilities: checkRoleCapabilities(role, profile).reported,
    };
  }
  const independence = compareReviewerIndependence(protocol);
  return {
    version: resolved.version,
    mode: roles.executor ? 'autonomous-capable' : 'manual',
    roles,
    independence: {
      status: independence.status,
      nextAction: independence.status === 'distinct' ? null : REVIEWER_INDEPENDENCE_REMEDIATION,
    },
...
export function runDoctor(store) {
  ...
  const agentConfig = resolveAgentConfiguration(proto);
  ...
  const gitOk = commandOnPath('git'), ghOnPath = commandOnPath(ghCmd);
  ...
  // --- agents / executor ---
  for (const problem of agentConfig.problems) add('agents', 'fail', `${problem} — fix: chalk connect --replace --builder <adapter> --reviewer <adapter>`);
  for (const [role, profile] of Object.entries(agentConfig.roles)) {
    if (!profile) continue;
    const support = checkRoleCapabilities(role, profile);
    for (const problem of support.problems) add('agents', 'fail', `${problem} — fix: chalk connect --replace --builder <adapter> --reviewer <adapter>`);
    const identity = profile.identity?.independenceKey ? ` · identity ${profile.identity.independenceKey}` : ' · identity unknown';
    const caps = support.reported ? ` · capabilities access=${JSON.stringify(support.reported.access || support.reported.accessEnforced)} output=${JSON.stringify(support.reported.output || support.reported.structuredOutput)}${support.reported.roles ? ` roles=${JSON.stringify(support.reported.roles)}` : ''}` : '';
    add('agents', 'ok', `${role} → ${profile.name} (${profile.adapter}, ${profile.source})${identity}${caps}`);
  }
  const executorProfile = agentConfig.roles.executor;
  add('executor', executorProfile?.command ? 'ok' : 'fail',
    executorProfile?.command ? `executor bound to ${executorProfile.name} (${executorProfile.adapter})`
      : 'no protocol.executor.command — required for `chalk run`/`chalk pipeline`; OPTIONAL for the manual loop (chalk next → write code → chalk verify → chalk done), which works without one. Autonomous fix: `chalk connect --preset autonomous --builder <adapter> --reviewer <adapter>`');

  // --- gates wired ---
  const cadences = reviewCadences(proto.review || {});
  const reviewerProfile = agentConfig.roles.reviewer;
  if (cadences.length && !reviewerProfile?.command) add('gates', 'warn', `review cadence ${JSON.stringify(cadences)} set but no reviewer role is bound — fix: chalk connect --replace --builder <adapter> --reviewer <adapter>`);
  ...
  if (cadences.length && executorProfile?.command && reviewerProfile?.command) {
    const independence = compareReviewerIndependence(proto);
    if (independence.status === 'same') add('gates', 'warn', `P5 reviewer has the same explicit independence identity as the executor — fix: ${REVIEWER_INDEPENDENCE_REMEDIATION}`);
    else if (independence.status === 'distinct') add('gates', 'ok', 'P5 reviewer independence verified by distinct explicit identity keys');
    else add('gates', 'warn', `P5 reviewer independence unverified — ${REVIEWER_INDEPENDENCE_REMEDIATION}`);
  }
  ...
  const reviewerGate = !!reviewerProfile?.command && reviewCadences(proto.review).length > 0;
