...
export function runRetro(store) {
  ...
  let raw = '';
  const t0 = Date.now();
  try { raw = execSync(withJsonOutput(withRunner(store.protocol().runner, cmd)), { cwd: store.root, input: digest, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 10 * 60 * 1000, maxBuffer: 64 * 1024 * 1024 }); }
  catch (e) { raw = `${e.stdout || ''}${e.stderr || ''}`; }
  const { text, usage } = unwrapAgentOutput(raw); // envelope off BEFORE the parser (#99)
  store.logCost({ stage: 'retro', agent: 'retro', ms: Date.now() - t0, ...(usage || {}) });
  const o = parseLastJson(text, (x) => Array.isArray(x.lessons) || Array.isArray(x.issues));
