...
export function runRetro(store) {
  ...
  let raw = '';
  const t0 = Date.now();
  try { raw = execSync(withJsonOutput(withRunner(store.protocol().runner, cmd)), { cwd: store.root, input: digest, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], timeout: 10 * 60 * 1000 }); }
