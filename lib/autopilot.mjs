...
export function runAutopilot(store, cliPath, { max = 3, retro = true, minSeverity = 'med', log = () => {} } = {}) {
  ...
  try {
    ...
    if (retro && resolveAgentRole(store.protocol(), 'retro')?.command) {
      log('retro: distilling lessons + filing issues');
      try { spawnSync('node', [cliPath, 'retro', '--min-severity', String(minSeverity)], { cwd: store.root, encoding: 'utf8' }); } catch { /* retro is best-effort — never fail an otherwise-successful sweep */ }
    }
