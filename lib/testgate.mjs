...
// `chalk pr` (the manual-order flows the pipeline's commit stage doesn't sweep). Scope matches
// what `chalk commit` sweeps: `.chalk/` pins (e2e specs) are excluded — spine state lands
// out-of-band by design, so the worktree legitimately never tracks it. In a non-git tree tracking
// is unverifiable, so nothing is reported — the other gates still apply.
export function untrackedLockedTests(store, task) {
  const pinned = (task.tests || []).map((t) => t.path).filter((p) => p && !p.startsWith('.chalk/'));
  if (!pinned.length) return [];
  const wd = workdir(store, task);
  if (gitTry(wd, 'rev-parse --is-inside-work-tree') !== 'true') return [];
  const tracked = new Set(gitTry(wd, 'ls-files -z').split('\0').filter(Boolean));
  return pinned.filter((p) => !tracked.has(p));
}
