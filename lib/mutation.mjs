...
export function runMutation(store, task, { cwd = workdir(store, task), strictExecution = false } = {}) {
  ...
  const state = new Map();
  for (const p of impl) {
    const result = launchShellCommand(withRunner(runner, tmpl.replace('{file}', p)), { cwd, stdio: 'pipe', timeout: 20 * 60 * 1000 });
    ...
    state.set(p, result.status === 0 && !result.error ? 'clean' : unrunnable ? 'error' : 'survived');
  }
  return {
    skipped: false,
