...
export function reviewComment({ verdict, findings = [], specRevision }) {
  const scope = specRevision === undefined ? 'Required Chalk gates still decide merge.' : `Applies only to specification revision ${specRevision}. Any amendment invalidates this review. Required Chalk gates still decide merge.`;
  if (verdict === 'pass') {
...
export function postReviewToPr(store, task, { verdict, findings = [] }) {
  ...
  const publish = () => {
    ...
    runGh(workdir(store, task), ghCmd, `pr comment ${num} --body-file -`, { input: reviewComment({ verdict, findings, specRevision: task.specRevision || 0 }) });
