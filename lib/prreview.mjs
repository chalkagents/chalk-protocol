...
export function reviewComment({ verdict, findings = [], specRevision, sourceDigest }) {
  const scope = (specRevision === undefined ? 'Required Chalk gates still decide merge.' : `Applies only to specification revision ${specRevision}. Any amendment invalidates this review. Required Chalk gates still decide merge.`) + (sourceDigest ? `\n\nReviewed source identity: \`${sourceDigest}\`. Changed inputs require a fresh review.` : '');
  if (verdict === 'pass') {
    return `### 🤖 Chalk adversarial review — **${LGTM}** ✅\n\n` +
      `The change satisfies every acceptance criterion across correctness, test-adequacy, ` +
      `design-intent, and regression.\n\n${scope}`;
  }
...
export function postReviewToPr(store, task, { verdict, findings = [] }) {
  ...
  const publish = () => {
    ...
    if (!current || (current.specRevision || 0) !== (task.specRevision || 0)) return { posted: false, lgtm: false, reason: 'specification changed before review publication' };
    if (typeof store.task === 'function') requireCurrentApproval(store, 'review', task.reviews?.at(-1), task);
    runGh(workdir(store, task), ghCmd, `pr comment ${num} --body-file -`, { input: reviewComment({ verdict, findings, specRevision: task.specRevision || 0, sourceDigest: task.reviews?.at(-1)?.approval?.source }) });
    return { posted: true, lgtm: verdict === 'pass' };
  };
  try {
    return typeof store.withLock === 'function' ? store.withLock(publish, { protectOwner: true }) : publish();
  } catch (e) {
