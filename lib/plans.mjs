...
const STATE_COLUMN = { todo: 'todo', specd: 'todo', 'in-progress': 'inprogress', blocked: 'inprogress', done: 'done' };
...
function frontmatter(task) {
  ...
  const status = completionCurrent(task) ? 'done' : 'pending';
...
function body(task, phase) {
  ...
  if (task.state === 'done' && !completionCurrent(task)) out.push(`Current contract needs revalidation: \`chalk start ${task.id}\`. Historical completion is retained in revision history.`, '');
...
export function projectPlans(store) {
  ...
  tasks.forEach((task, i) => {
    const column = task.state === 'done' && !completionCurrent(task) ? 'todo' : STATE_COLUMN[task.state] || 'todo';
