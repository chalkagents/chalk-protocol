...
import { compareReviewerIndependence, resolveAgentConfiguration, REVIEWER_INDEPENDENCE_REMEDIATION } from './config.mjs';
...
export function connectionReadiness(protocol, discoveries) {
  ...
  if (resolved.roles.executor && resolved.roles.reviewer) {
    ...
    else checks.push({ level: 'warn', message: independence.status === 'same' ? 'builder and reviewer use the same configured identity' : 'builder/reviewer independence cannot be verified', nextAction: REVIEWER_INDEPENDENCE_REMEDIATION });
  }
