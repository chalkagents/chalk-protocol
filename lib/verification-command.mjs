...
public static class ChalkWindowsJob {
  ...
  public static int Run(string commandInterpreter, string script, string cwd) {
...
try { exit [ChalkWindowsJob]::Run($env:ComSpec, $env:CHALK_VERIFY_SCRIPT, (Get-Location).Path) }
...
async function ownWindowsCommand(requestPath) {
  ...
  try {
    ...
    command = spawn('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', request.runner], {
      cwd: request.cwd, windowsHide: true, env: { ...process.env, CHALK_VERIFY_SCRIPT: request.script }, stdio: ['ignore', 3, 4],
    });
...
async function supervise(request) {
  ...
  try {
    outcome = await new Promise(resolveResult => {
      ...
      if (process.platform === 'win32') {
        const commandScript = join(spool, 'command.cmd'), jobRunner = join(spool, 'windows-job.ps1');
        writeFileSync(commandScript, `@echo off\r\n${request.cmd}\r\nexit /b %errorlevel%\r\n`, { mode: 0o600 });
        writeFileSync(jobRunner, WINDOWS_JOB_RUNNER, { mode: 0o600 });
        const commandRequest = join(spool, 'command.json');
        writeFileSync(commandRequest, JSON.stringify({ cwd: root, script: commandScript, runner: jobRunner }), { mode: 0o600 });
