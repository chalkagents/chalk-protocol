...
import { readFileSync, writeSync, openSync, closeSync, rmSync, mkdirSync, mkdtempSync, watch, lstatSync, readdirSync, fstatSync } from 'node:fs';
...
function monitorInputs(root, proto, changed, failed, source) {
  ...
  try {
    ...
    try {
      ...
      w.on('error', error => failed(error.message)); watchers.push(w); recursiveSource = true;
    } catch (error) {
    ...
    for (const path of [...Object.keys(initial.policyInputs || {}), ...Object.keys(initial.membershipInputs || {})]) {
      ...
      // Recursive source events already retain internal .gitignore and repository-locator
      // transitions. Keep explicit watches for external/ancestor policies and .git internals.
      if (recursiveSource && withinRoot && (membership?.kind === 'locator' || internalIgnore)) continue;
      let parent = dirname(path);
      while (true) {
        try { if (lstatSync(parent).isDirectory()) break; throw new Error('invalid policy parent'); }
...
async function supervise(request) {
  ...
  const running = { gate: request.gate, status: 'running', cmd: request.cmd, startedAt, stdoutPath: liveOut, stderrPath: liveErr };
  ...
  const parentCheck = setInterval(() => {
    try { process.kill(request.parentPid, 0); } catch (e) { if (e.code === 'ESRCH') cancel(); }
  ...
  try {
    outcome = await new Promise(resolveResult => {
      ...
      child = spawn(request.cmd, { cwd: root, shell: true, detached: process.platform !== 'win32', stdio: ['ignore', 'pipe', 'pipe'] });
      ...
      deadline = setTimeout(() => { timedOut = true; killTree(); boundCancellation(); }, request.timeoutMs ?? 600000);
