...
function monitorInputs(root, proto, changed, failed, source) {
  ...
  try {
    ...
    for (const [parent, paths] of policies) {
      ...
      if (process.platform === 'win32' && selectedDirectories.some(dir => parent === dir || parent.startsWith(dir + sep))) continue;
...
async function ownWindowsCommand(requestPath) {
  ...
  try {
    ...
    command = spawn(request.cmd, { cwd: request.cwd, shell: true, windowsHide: true, stdio: ['ignore', 3, 4] });
...
if (process.argv[1] && resolve(process.argv[1]) === SELF) {
  if (process.argv[2] === '--own-windows-command') await ownWindowsCommand(process.argv[3]);
    ...
    // writeFileSync uses Node's direct descriptor binding. That keeps the in-memory
    // result channel available when a capture-failure test replaces fs.writeSync.
    try { writeFileSync(1, JSON.stringify(await supervise(JSON.parse(readFileSync(0, 'utf8'))))); }
