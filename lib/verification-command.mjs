...
function monitorInputs(root, proto, changed, failed, source) {
  ...
  if (process.platform === 'win32') {
    // ReadDirectoryChangesW keeps watched repository/configuration directories open,
    // which prevents commands from renaming a .git selector. Poll exact authority
    // identities instead: it retains transient changes without long-lived handles.
    authorityTimer = setInterval(() => observe(checkAuthorities), 25);
  }
  const check = async () => {
...
async function ownWindowsCommand(requestPath) {
  ...
  const report = result => {
    ...
    if (process.connected) process.send({ type: 'result', ...result }, closeOutputs);
    else closeOutputs();
  };
  process.on('message', message => { if (message?.type === 'finish') process.exit(0); });
  process.on('disconnect', () => {
    if (command?.pid) spawnSync('taskkill', ['/pid', String(command.pid), '/T', '/F'], { stdio: 'ignore' });
    process.exit(1);
  });
  try {
    // The owner remains a live ancestor after cmd.exe exits. Closing only its output
    // handles lets inherited descendants drain naturally, while detached/ignored
    // workers do not block a later verification gate.
    command = spawn(request.cmd, { cwd: request.cwd, shell: true, windowsHide: true, stdio: ['ignore', 1, 2] });
  ...
  try {
    outcome = await new Promise(resolveResult => {
      ...
      if (process.platform === 'win32') {
        ...
        writeFileSync(commandRequest, JSON.stringify({ cmd: request.cmd, cwd: root }), { mode: 0o600 });
      ...
      if (process.platform === 'win32') {
        ...
        child.stdout.once('end', () => { ended++; maybeFinish(); });
        child.stderr.once('end', () => { ended++; maybeFinish(); });
