...
export function verificationCoverage(result, mode = 'task') {
  ...
  return {
    ...
    review: 'not-evaluated', release: 'not-evaluated', receiptId: result.evidence?.id || null, reused: result.reused === true,
  };
}

export function auditCoverage(result) {
  const executed = result.regressionExecuted ?? result.status !== 'unconfigured';
  return {
    green: result.green === true,
    phase: verificationCoverage(result.phaseVerification, 'phase'),
    heldOut: {
      status: executed ? (result.passed ? 'passed' : 'failed') : result.status === 'unconfigured' ? 'unconfigured' : 'not-executed', executed,
