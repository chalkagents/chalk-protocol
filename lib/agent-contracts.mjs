...
export function decodeStructuredRole(role, text) {
  ...
  // A missing complete object commonly means the provider stream was truncated. Reviewer workflows
  // may retry this documented transient once; a complete object with the wrong schema stays terminal.
  if (!value) return { structured: null, diagnostic: { level: 'error', code: 'malformed-structured-output', message: 'agent output did not contain one complete JSON object', retryable: true } };
...
export function checkRoleCapabilities(role, profile) {
  ...
  const roles = Array.isArray(declared.roles) ? declared.roles.map(String) : null;
  const problems = [];
  if (roles && !roles.includes(role)) problems.push(`${role} is not supported by profile ${profile.name}`);
  ...
  return { ok: problems.length === 0, problems, reported: { access, output, ...(roles ? { roles } : {}) } };
}
