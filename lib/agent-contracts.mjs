...
export function decodeStructuredRole(role, text) {
  ...
  if (!valid(role, value)) return { structured: null, diagnostic: { level: 'error', code: 'schema-invalid', message: `agent output does not satisfy ${ROLE_CONTRACTS[role]?.output?.schema || `${role} contract`}`, retryable: false } };
