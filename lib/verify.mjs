...
import { runVerificationCommand, startVerificationMonitor, DEFAULT_VERIFICATION_TIMEOUT_MS } from './verification-command.mjs';
...
export { DEFAULT_VERIFICATION_TIMEOUT_MS } from './verification-command.mjs';
...
export function runToolchain(root, verifyConfig = {}, { runner = '', mode = 'task', timeoutMs = DEFAULT_VERIFICATION_TIMEOUT_MS, runDir, receiptPath, storage, proto = {}, monitorSource, onResult = () => {}, runCommand = runVerificationCommand } = {}) {
  const results = [];
  ...
  for (const gate of GATES) {
    ...
    onResult(results);
    const result = { ...runCommand({ cwd: root, gate, cmd, timeoutMs, stdoutPath, stderrPath, receiptPath, storage, proto, monitorSource }), when: g.when };
...
export function verify(store, { mode = 'task', cwd = store.root } = {}) {
  ...
  try {
    ...
    const result = performVerify(store, { mode, cwd, inputs: record.before, monitorSource, runDir: evidence.dir, receiptPath: evidence.path, storage: evidence.storage, onResult: toolchain => {
    ...
    const green = result.green && freshness === 'fresh' && !evidenceError;
    ...
    return { green: false, toolchain: receipt?.record.toolchain || [], integrity: [], e2e: [], toolchainGreen: false, integrityGreen: true, e2eGreen: true, evidence: receipt?.evidence, evidenceError: e.message, freshness: 'unknown' };
  }
}
...
function performVerify(store, { mode, cwd, inputs, monitorSource, runDir, receiptPath, storage, onResult }) {
  ...
  let specIndex = 0;
  const e2e = specPaths.length && proto.e2e?.command ? runSpecs(store, cwd, specPaths, { protocol: proto, timeoutMs: DEFAULT_VERIFICATION_TIMEOUT_MS, execute: (cmd, options) => {
    const gate = `e2e-${specIndex++}`;
    return runVerificationCommand({ cwd, gate, cmd, timeoutMs: options.timeoutMs, stdoutPath: join(runDir, `${gate}.stdout.log`), stderrPath: join(runDir, `${gate}.stderr.log`), receiptPath, storage, section: 'browserCommands', proto, monitorSource });
  } }) : [];
