...
import { captureVerificationProvenance, verificationReuseInputs, saveVerificationReferences } from './verification-reuse.mjs';
...
export function runToolchain(root, verifyConfig = {}, { runner = '', mode = 'task', timeoutMs = 10 * 60 * 1000, runDir, receiptPath, storage, proto = {}, monitorSource, onResult = () => {} } = {}) {
  ...
  for (const gate of GATES) {
    ...
    onResult(results);
    const result = { ...runVerificationCommand({ cwd: root, gate, cmd, timeoutMs: g.timeoutMs ?? timeoutMs, stdoutPath, stderrPath, receiptPath, storage, proto, monitorSource }), when: g.when };
...
export function verify(store, { mode = 'task', cwd = store.root, preflightError } = {}) {
  ...
  try {
    ...
    record.provenance = { before: captureVerificationProvenance(record.before?.config || {}, mode, process.env, cwd) };
    evidence.receiptDigest = saveVerificationRecord(evidence, record);
    try { saveVerificationReferences(store, record, evidence); } catch { /* a missing locator only disables later reuse */ }
    ...
    record.provenance.after = captureVerificationProvenance(after.config, mode, process.env, cwd);
    ...
    const freshness = observation.monitorError || commands.some(r => r.monitorError) ? 'unknown'
      : observation.inputChanges.length || commands.some(r => r.inputChanges?.length) ? 'stale'
      : inputsFresh(record.before, after);
    ...
    const reuseInputs = verificationReuseInputs(after, record.provenance);
    Object.assign(record, { ...result, green, after, freshness, reuseInputs, status: evidenceError ? 'error' : 'complete', finishedAt });
    try { evidence.receiptDigest = saveVerificationRecord(evidence, record); }
    ...
    try { saveVerificationReferences(store, record, evidence); } catch { /* a missing locator only disables later reuse */ }
    return { ...result, green, evidence, freshness, before: record.before, after, mode, provenance: record.provenance, reuseInputs };
  } catch (e) {
    monitor?.close();
    if (receipt) {
      ...
      try {
        receipt.evidence.receiptDigest = saveVerificationRecord(receipt.evidence, receipt.record);
        saveVerificationReferences(store, receipt.record, receipt.evidence);
      } catch { /* original storage error remains authoritative */ }
    }
