...
import { reviewEvidence, formatPrEvidence } from './review-evidence.mjs';
...
export function prNarrative(store, task, changed = []) {
  const profile = resolveAgentRole(store.protocol(), 'pr-narrative');
...
export function buildPrBody(store, task, { changed = [], narrative = '' } = {}) {
  ...
  out.push('## Test plan', '- Run `chalk verify` for the configured checks and locked-test integrity.');
  if (locked) out.push('- locked tests:', locked);
  out.push('', formatPrEvidence(reviewEvidence(store, task, workdir(store, task))));
