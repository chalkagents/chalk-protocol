...
export const CLAUDE_ADAPTER_MANIFEST = Object.freeze({
  id: 'claude', displayName: 'Claude Code', binary: 'claude', roles: CLAUDE_SUPPORTED_ROLES,
  capabilities: { access: ['read-only', 'workspace-write'], output: ['text', 'json', 'none'] },
  command: `${JSON.stringify(process.execPath)} ${JSON.stringify(fileURLToPath(new URL('../../bin/adapters/claude.mjs', import.meta.url)))}`,
  installCommand: 'npm install -g @anthropic-ai/claude-code', authCommand: 'claude',
  probe: (options) => probeCli({ id: 'claude', displayName: 'Claude Code', binary: 'claude', versionArgs: ['--version'], authProbeArgs: ['auth', 'status'], installCommand: 'npm install -g @anthropic-ai/claude-code', authCommand: 'claude' }, options),
});
