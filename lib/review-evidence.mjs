...
export function reviewEvidence(store, task, cwd = store.root) {
  try {
    ...
    const freshness = {
      ...
      spec: digest(verificationTaskContract(task)) === digest(verificationTaskContract(contract)) ? 'fresh' : 'stale',
    ...
    const summarized = commands.map(command => ({
      ...
      startedAt: text(command.startedAt, 80), finishedAt: command.status === 'running' ? null : text(command.finishedAt, 80),
    ...
    return {
      ...
      startedAt: text(record.startedAt, 80), finishedAt: text(record.finishedAt, 80), freshness,
      runtime: { recordedNode: text(record.runtime?.node, 80), recordedPlatform: text(record.runtime?.platform, 40), completeToolchainIdentity: 'not established' },
...
export function formatPrEvidence(summary) {
  let data = {
    ...
    startedAt: summary.startedAt || null, finishedAt: summary.finishedAt || null,
    commands: (summary.commands || []).slice(0, COMMAND_LIMIT).map(command => ({
      gate: command.gate, status: command.status, exitCode: command.exitCode,
      startedAt: command.startedAt, finishedAt: command.finishedAt, durationMs: command.durationMs,
    })),
