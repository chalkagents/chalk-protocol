...
export function buildContext(store, task) {
  ...
  const lessonsBlock = fitLessonsBlock(store.lessons(), budget - essentialBytes - directorBytes);
  ...
  const raiseFits = task && (budget - essentialBytes - directorBytes - lessonsBytes) > Buffer.byteLength(RAISE_INSTRUCTION, 'utf8') + 1;
  const raiseBlock = raiseFits ? RAISE_INSTRUCTION : null;
  return [header, specSection, ...(directorBlock ? [directorBlock] : []), ...(lessonsBlock ? [lessonsBlock] : []), ...(raiseBlock ? [raiseBlock] : []), ...post].join('\n');
}
