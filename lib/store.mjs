...
import { existsSync, mkdirSync, readFileSync, writeFileSync, appendFileSync, statSync, realpathSync, rmSync, renameSync, readdirSync } from 'node:fs';
import { dirname, join, resolve, relative } from 'node:path';
import { fileURLToPath } from 'node:url';
import { currentCriteria, completionCurrent } from './spec-revisions.mjs';
import { archivedTasks } from './task-history.mjs';
import { PRESETS } from './config.mjs';
import { providerScaffold } from './provider-presets.mjs';
...
export const MIGRATIONS = [
  ...
  { from: '1.1', to: '1.2', describe: 'add provider-neutral agent profiles and role bindings',
    apply: (meta) => {
      meta.protocol = meta.protocol || {};
      // Do not copy legacy commands: runtime compatibility profiles resolve the CURRENT values, so
      // user edits made before or after migration remain authoritative and are never overwritten.
      meta.protocol.agents = meta.protocol.agents || { version: 1, profiles: {}, roles: {} };
    } },
];
...
export const SPINE_STATE_PATHS = [
  ...
  '.chalk/decisions.md', '.chalk/director.jsonl', '.chalk/lessons.md', '.chalk/skills', '.chalk/boards', '.chalk/plans',
  '.chalk/handoffs', '.chalk/analysis', '.chalk/archive',
];
...
export const protocolPath = (value) => String(value || '').split('\\').join('/');
  ...
  while (true) {
    ...
    if (existsSync(dotgit)) {
      ...
      try {
        ...
        return { toplevel: realPath(dir), mainRoot: realPath(dirname(common)) };
      } catch { return null; }
    }
    const parent = dirname(dir);
    if (parent === dir) return null;
...
export function findRoot(start = process.cwd()) {
  ...
  const wt = linkedWorktree(realPath(start));
  if (wt) {
    const rel = relative(wt.toplevel, realPath(local || start));
    if (!rel.startsWith('..')) {
...
const paths = (root) => {
  ...
  return {
    ...
    skills: join(base, 'skills'), // first-class project how-to (#215), injected into agent context
...
// Synchronous sleep for short filesystem/process retry loops — Atomics.wait blocks the thread without
// a busy spin (the store API is entirely synchronous, so we cannot yield to the event loop here).
const sleepSync = (ms) => { try { Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms); } catch { /* SAB unavailable — fall through */ } };
// Windows can transiently refuse replace-style rename while a reader or scanner still has the target
// open. Retrying the SAME atomic replace preserves the old-or-new reader contract; deleting the target
// first would create a missing-file window and is deliberately forbidden.
export const replaceFileSync = (from, to, {
  ...
  for (;;) {
    try { rename(from, to); return; }
      ...
      if (platform !== 'win32' || !['EACCES', 'EPERM', 'EBUSY'].includes(error.code) || nowMs() >= deadline) throw error;
      sleep(10);
    }
  }
};
...
const lockGeneration = (lockPath) => {
  try {
    const stat = statSync(lockPath);
    if (stat.isDirectory()) {
      try {
        const owner = readFileSync(join(lockPath, 'owner'), 'utf8');
        const token = owner.split(' ')[0];
        const protectedPid = Number(owner.match(/\bprotected=(\d+)/)?.[1]) || null;
        if (token) return { token, protectedPid, mtimeMs: stat.mtimeMs, kind: 'directory', owned: true };
      } catch { /* Chalk may have crashed after mkdir but before its exclusive owner claim */ }
      // Compatibility recovery for an ownerless directory left by the earlier two-step protocol.
      // Its filesystem identity makes every waiter choose the same retirement tombstone.
      return { token: `orphan-${stat.dev}-${stat.ino}`, mtimeMs: stat.mtimeMs, kind: 'directory', owned: false };
    }
    try {
      const token = readFileSync(lockPath, 'utf8').split(' ')[0];
      if (token) return { token, mtimeMs: stat.mtimeMs, kind: 'file', owned: true };
    } catch { /* use filesystem identity for an unreadable/empty legacy lock */ }
    return { token: `legacy-${stat.dev}-${stat.ino}`, mtimeMs: stat.mtimeMs, kind: 'file', owned: true };
  } catch { return null; }
};

const retirementPath = (lockPath, token) => {
  const retired = join(dirname(lockPath), '.locks');
  mkdirSync(retired, { recursive: true });
  // Existing Chalk projects may not have the newer root ignore rules. A self-ignored lock directory
  // keeps permanent ABA guards invisible without dirtying the host repository during an upgrade. An
  // atomic replacement repairs a zero-byte file left by interruption between create and write.
  const ignore = join(retired, '.gitignore');
  let validIgnore = false;
  try { validIgnore = readFileSync(ignore, 'utf8') === '*\n'; } catch { /* initialize below */ }
  if (!validIgnore) {
    const pending = `${ignore}.${process.pid}.${randomUUID()}.tmp`;
    try { writeFileSync(pending, '*\n'); replaceFileSync(pending, ignore); }
    finally { rmSync(pending, { force: true }); }
  }
  return join(retired, sha256(String(token)));
};

// Generation-specific retirement is the ABA guard for normal release and stale takeover. Every actor
// that observed generation T renames it to the SAME persistent tombstone. A live directory contains
// an owner file, so its tombstone is non-empty: a delayed T actor cannot rename a fresh T+1 directory
// over it on POSIX or Windows. Legacy file locks retire to a file; current replacements are directories
// and likewise cannot overwrite that destination. The tiny markers live under self-ignored .locks/.
export function retireLockGeneration(lockPath, expectedToken, { requireStale = false, nowMs = Date.now() } = {}) {
  const current = lockGeneration(lockPath);
  if (!current || current.token !== expectedToken) return false;
  if (requireStale && nowMs - current.mtimeMs <= LOCK_STALE_MS) return false;
  if (requireStale && current.protectedPid) {
    // Network-backed admission may legitimately outlive the ordinary lease.
    // Only a confirmed dead owner permits takeover; EPERM is not proof of death.
    try { process.kill(current.protectedPid, 0); return false; }
    catch (error) { if (error.code !== 'ESRCH') return false; }
  }
  const retired = retirementPath(lockPath, expectedToken);
  if (existsSync(retired)) return false;
  try { renameSync(lockPath, retired); return true; }
  catch (error) {
    if (error.code === 'ENOENT' || existsSync(retired)) return false;
    throw error;
  }
}
...
export class Store {
  ...
  // tasks.json — last writer wins, silently dropping the other's update. Atomic directory creation
  // plus an exclusive owner-file claim is the portable create-if-absent primitive: one contender wins,
  // stealing a lock whose holder crashed (older than LOCK_STALE_MS) so a dead process can't wedge the
  // spine forever. Timing out fails closed rather than proceeding unlocked and corrupting state.
  // Synchronous and non-reentrant: never call withLock from inside a withLock body.
  withLock(fn, { protectOwner = false } = {}) {
    const lockPath = join(this.p.base, '.lock');
    const start = Date.now();
    const token = `${process.pid}-${randomUUID()}`;
    const contention = (error, path) => error.code === 'EEXIST'
      || (process.platform === 'win32' && ['EACCES', 'EPERM', 'EBUSY'].includes(error.code) && existsSync(path));
    let held = false;
    for (;;) {
      try {
        mkdirSync(lockPath);
        // A delayed creator may reach a replacement directory after its own directory was retired.
        // Exclusive owner creation means it can only take that generation if nobody claimed it yet;
        // otherwise it becomes a waiter and can never overwrite the replacement owner's identity.
        writeFileSync(join(lockPath, 'owner'), `${token} ${now()}${protectOwner ? ` protected=${process.pid}` : ''}`, { flag: 'wx' });
        held = true;
        break;
      }
      catch (e) {
        if (!contention(e, lockPath)) throw e;
        const generation = lockGeneration(lockPath);
        if (!generation) continue; // released between creation failure and inspection — retry now
        if (Date.now() - generation.mtimeMs > LOCK_STALE_MS) {
          if (!generation.owned && generation.kind === 'directory') {
            // Claim a directory left by a crash around owner creation. A pre-existing zero-byte
            // owner already makes it non-empty; otherwise wx establishes the stable inode-derived
            // claim. Either form can retire without a delayed actor replacing its tombstone.
            const ownerPath = join(lockPath, 'owner');
            let retireReady = existsSync(ownerPath); // includes an interrupted zero-byte owner file
            if (!retireReady) {
              try { writeFileSync(ownerPath, generation.token, { flag: 'wx' }); retireReady = true; }
              catch (claimError) { if (!contention(claimError, ownerPath)) throw claimError; }
            }
            if (retireReady && retireLockGeneration(lockPath, generation.token)) continue;
          } else if (retireLockGeneration(lockPath, generation.token, { requireStale: true })) continue;
        }
        if (Date.now() - start > LOCK_WAIT_MS) {
          const timeout = new Error(`Timed out waiting ${LOCK_WAIT_MS}ms for the Chalk spine lock`);
    ...
    try { return fn(); }
      ...
      if (held) {
        ...
        if (current?.token === token) {
          // Release and stale takeover use the SAME non-empty generation tombstone. If this owner
          // pauses while a waiter replaces T with T+1, T's tombstone makes the late rename fail rather
          // than replacing T+1. Removing the tombstone would re-open that delayed-actor ABA race.
          retireLockGeneration(lockPath, token);
        }
      }
    }
  }
  ...
  upsertTask(task, { admit } = {}) {
    this.mutateTasks((tasks) => {
      const i = tasks.findIndex((t) => t.id === task.id);
      // Gate writers may admit and merge their result against the current record
      // inside this transaction, rather than replacing concurrent task history.
      if (admit) task = admit(i >= 0 ? tasks[i] : undefined, task);
      if (i >= 0) {
        // A command may have held this object across an executor/reviewer run.
        // Never let that stale snapshot undo an intervening sanctioned amendment.
        if ((tasks[i].specRevision || 0) !== (task.specRevision || 0)) throw new Error('specification changed while this command was running — reload chalk context and retry against the current revision');
        // Every whole-task writer must preserve recorded gate verdicts, even
        // outside approval commands (planning, branching, blocking, triage).
        // Director decision annotations are mutable; the review itself is not.
        const gateRecord = ({ decisions, ...record }) => record;
        const previous = (tasks[i].reviews || []).map(gateRecord), proposed = (task.reviews || []).map(gateRecord);
        if (!isDeepStrictEqual(previous, proposed.slice(0, previous.length))) throw new Error(`review history changed while this command was running — reload chalk context and run chalk review ${task.id}`);
        if (tasks[i].state === 'done' && !completionCurrent(tasks[i]) && task.state !== 'done') {
          // Every reopening path starts a new validation/release cycle. The old
          // candidate remains in revision history, never in active shipping fields.
          for (const key of ['doneAt', 'released', 'releasedAt', 'branch', 'worktree', 'pr']) delete task[key];
          task.pipeline = { stage: 'selected', verificationInvalidated: task.specRevision, planInvalidated: task.pipeline?.planInvalidated };
        }
        tasks[i] = task;
      } else {
        if (archivedTasks(this, { strict: true }).some(archived => archived.id === task.id)) throw new Error('task was archived while this command was running — cannot resurrect its stale snapshot; reload chalk context');
        task.acceptanceCriteria = currentCriteria(task); tasks.push(task);
      }
      return tasks;
    }, { protectOwner: !!admit });
    return task;
  ...
  lockTest(absPath) {
    ...
    return { path: protocolPath(relative(this.lockBase(dirname(absPath)), absPath)), sha256: sha256(readFileSync(absPath)) };
  }
  ...
  // --- append-only logs ---
  emitUpdate({ type = 'progress-update', title, description = '', phase, actorRole = 'agent', taskId, verification, audit }) {
  ...
  // --- project skills (#215): first-class reusable how-to ("how this project does X"), authored as
  // .chalk/skills/<name>.md and injected into every agent's context. Distinct from lessons (mistakes not
  // to repeat) — skills are the affirmative playbook. Just injected text, never executable. ---
  skills() {
    if (!existsSync(this.p.skills)) return [];
    return readdirSync(this.p.skills).filter((f) => f.endsWith('.md')).sort()
      .map((f) => ({ name: f.replace(/\.md$/, ''), text: readFileSync(join(this.p.skills, f), 'utf8') }));
  }
...
// A task's OPEN mid-flight raises (#209) — forks the agent surfaced (chalk raise) instead of guessing,
// still awaiting the director's answer.
export const openRaises = (task) => (task.raised || []).filter((r) => r && r.status === 'open');
// A task is awaiting rework when it was RE-OPENED (reopenedAt, set only on a done→redirect) and still
// has pending corrections. Keying on reopenedAt — not merely "in-progress + a directive" — is what keeps
// a redirect on an ALREADY-active task from being re-admitted to the driver and double-executed (#200).
export const needsRework = (task) => !!task.reopenedAt && pendingDirectives(task).length > 0;
// Mark all of a task's pending corrections resolved (the rework landed); returns how many. Shared by
// every completion path (`chalk done`, pipeline merge) so directive-resolution can't diverge.
export function resolveDirectives(task) {
  const pend = pendingDirectives(task);
  const at = now();
  for (const d of pend) { d.resolved = true; d.resolvedAt = at; }
  // Clear the re-open marker once the rework lands, so it can never coexist with a NEW directive added
  // while the task is later active — which would otherwise re-admit an in-flight task (the double-run
  // case needsRework exists to exclude). reopenedAt is set only on a done→redirect, so this keeps the
  // invariant "reopenedAt ⇒ awaiting the rework of a done→reopen" true for a task's whole lifetime.
  if (pend.length) delete task.reopenedAt;
  return pend.length;
}

export function runnableTasks(tasks) {
  // specd tasks are runnable; so is a RE-OPENED task (#200) awaiting rework, which the driver must
  // re-execute so the redirect actually gets reworked. An in-progress task that is NOT a re-open (a plain
  // WIP, or a redirect on an active task) is never re-admitted, so ordinary work can't be double-run.
  return tasks.filter((t) => (t.state === 'specd' || needsRework(t)) && depsSatisfied(t, tasks));
}
...
// Render the durable director-decision record (#201) as context lines for the compounding feed (#202):
// an accepted call says "apply this rationale", a redirected one says "do this instead" — the two are
// distinct on purpose (that is why B1 split rationale/instruction).
function directorLines(decisions) {
  return decisions.map((r) => {
    if (r.verdict === 'redirected') return `- redirected: "${r.choice || 'a choice'}" → ${r.instruction || '(see record)'}`;
    if (r.verdict === 'answered') return `- answered: "${r.choice || 'a fork'}" → ${r.instruction || '(see record)'}`; // an answered mid-flight raise (#211)
    return `- accepted: "${r.choice || 'a choice'}"${r.rationale ? ` (because ${r.rationale})` : ''}`;
  });
}
// The compounding taste feed (#202): a bounded block of the director's prior accept/redirect calls, so
// the agent applies past judgment on new work and the same fork stops recurring — the moat. Elastic like
// lessons: fit most-recent-first into `remaining` bytes, elide the rest with a note. Null when empty.
function fitDirectorBlock(decisions, remaining) {
  if (!decisions.length) return null;
  const heading = "## Director's calls so far (apply this taste — the human accepted/redirected these)";
  const lines = directorLines(decisions);
  const bytes = (s) => Buffer.byteLength(s, 'utf8');
  const kept = [];
  let used = bytes(heading) + 1;
  for (let i = lines.length - 1; i >= 0; i--) { // newest first under budget pressure
    const cost = bytes(lines[i]) + 1;
    if (remaining > 0 && used + cost <= remaining) { kept.unshift(lines[i]); used += cost; } else break;
  }
  if (!kept.length) return null;
  const elided = lines.length - kept.length;
  const note = elided ? `\n  _(${elided} older director call(s) elided to fit the context budget)_` : '';
  return `${heading}\n${kept.join('\n')}${note}\n`;
}

// The project-skills block (#215): the affirmative playbook injected into agent context, bounded like
// the other elastic blocks — fit whole skills (name-ordered) into `remaining`, elide extras with a note.
function fitSkillsBlock(skills, remaining) {
  if (!skills.length) return null;
  const heading = '## Project skills (apply these — how this project does things)';
  const bytes = (s) => Buffer.byteLength(s, 'utf8');
  const rendered = skills.map((sk) => `### ${sk.name}\n${sk.text.trim()}`);
  const kept = [];
  let used = bytes(heading) + 1;
  for (let i = 0; i < rendered.length; i++) {
    const cost = bytes(rendered[i]) + 1;
    if (remaining > 0 && used + cost <= remaining) { kept.push(rendered[i]); used += cost; } else break;
  }
  if (!kept.length) return null;
  const elided = rendered.length - kept.length;
  const note = elided ? `\n_(${elided} more skill(s) elided to fit the context budget)_` : '';
  return `${heading}\n${kept.join('\n\n')}${note}\n`;
}

// The mid-flight raise instruction (#210), injected as a low-priority context block so a shell-capable
// executor turns a silently-guessed fork into a director decision. Terse — the full convention is in the
// chalk-executor definition.
const RAISE_INSTRUCTION = "## Fork that needs the director's taste? RAISE it — don't guess\nHit a real judgment call the criteria don't answer? Run `chalk raise \"<the fork>\" [--options \"a|b|c\"]` instead of picking one — it pauses for the director. Raise only the few that need taste.\n";
...
export function buildContext(store, task) {
  ...
  if (task) {
    ...
    currentCriteria(task).forEach((c, i) => post.push(`  ${i + 1}. ${c.text}  (id: ${c.id})`));
    if (!(task.acceptanceCriteria || []).length) post.push('  (none yet — add with `chalk spec`)');
    if (task.state === 'done' && !completionCurrent(task)) post.push(`\nHistorical completion does not cover the current contract. Run \`chalk start ${task.id}\` to revalidate; dependencies and release remain gated.`);
  ...
  // The director's explicit calls are higher-signal taste than auto-collected lessons, so they get the
  // leftover budget first; lessons fill what remains. Both are elided-with-a-note under pressure; the
  // task essentials in `post` are never displaced (#202).
  // Compound PRIOR taste — decisions from OTHER tasks. The current task's own corrections already ride
  // in the essential "Director corrections" block (#199); re-listing them here would double the signal.
  const priorDecisions = store.directorDecisions().filter((r) => !task || r.taskId !== task.id);
  const directorBlock = fitDirectorBlock(priorDecisions, budget - essentialBytes);
  const directorBytes = directorBlock ? Buffer.byteLength(directorBlock, 'utf8') + 1 : 0;
  // Project skills (#215) — affirmative playbook, higher-signal than auto-lessons, so budgeted before them.
  const skillsBlock = fitSkillsBlock(store.skills(), budget - essentialBytes - directorBytes);
  const skillsBytes = skillsBlock ? Buffer.byteLength(skillsBlock, 'utf8') + 1 : 0;
  const lessonsBlock = fitLessonsBlock(store.lessons(), budget - essentialBytes - directorBytes - skillsBytes);
  const lessonsBytes = lessonsBlock ? Buffer.byteLength(lessonsBlock, 'utf8') + 1 : 0;
  // The mid-flight raise instruction (#210): executor guidance, so LOWEST priority — present at any
  // realistic budget, first to yield under extreme pressure, so the essential/director/lessons budget
  // that locked context-budget tests pin stays untouched. (The full convention lives in the executor def.)
  const raiseFits = task && (budget - essentialBytes - directorBytes - skillsBytes - lessonsBytes) > Buffer.byteLength(RAISE_INSTRUCTION, 'utf8') + 1;
  const raiseBlock = raiseFits ? RAISE_INSTRUCTION : null;
  return [header, specSection, ...(directorBlock ? [directorBlock] : []), ...(skillsBlock ? [skillsBlock] : []), ...(lessonsBlock ? [lessonsBlock] : []), ...(raiseBlock ? [raiseBlock] : []), ...post].join('\n');
}
...
export function initSpine(root, { name, goal, preset, runner, executor }) {
  ...
  const meta = {
    ...
    protocol: {
      ...
      // Provider-neutral agent connections. Named profiles hold one adapter connection; roles bind
      // to them. Empty by default: legacy protocol.*.command values resolve as compatibility profiles.
  ...
  // Apply an opaque provider-owned compatibility + Protocol v1 preset. Legacy fields remain so an
  // older Chalk binary can still run the initialized spine; named bindings win in current versions.
  const agentPreset = providerScaffold(executor);
  if (agentPreset) {
    meta.protocol.executor.command = agentPreset.legacy.executor || '';
    meta.protocol.planner.command = agentPreset.legacy.planner || '';
    meta.protocol.retro.command = agentPreset.legacy.retro || '';
    meta.protocol.review = {
      ...meta.protocol.review,
      command: agentPreset.legacy.reviewer || '',
      ...(agentPreset.reviewRequiredAt ? { requiredAt: agentPreset.reviewRequiredAt } : {}),
    };
    meta.protocol.agents = { version: 1, profiles: agentPreset.profiles, roles: agentPreset.roles };
  }
