...
export function initSpine(root, { name, goal, preset, runner }) {
  ...
  const meta = {
    ...
    protocol: {
      ...
      // worktrees created at <dir>/<repo>-<branch-slug>. `include` = live paths copied from the
      // primary into a fresh worktree so it sees current spine state (held-out/local always excluded);
      // `setup` = a bootstrap command (pub get / npm ci) run once in the worktree before work/verify.
      worktree: { enabled: true, dir: '..', include: ['.chalk'], setup: '' },
