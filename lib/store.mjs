...
export class Store {
  ...
  appendDirectorDecision({ choice, why = '', risk, taskId, verdict, by = 'human' }) {
    const rec = { at: now(), verdict, choice: String(choice || '').trim(), why: String(why || '').trim(), ...(risk ? { risk } : {}), ...(taskId ? { taskId } : {}), by };
    appendFileSync(this.p.director, JSON.stringify(rec) + '\n');
    this.emitUpdate({ type: 'decision-logged', title: `Director ${verdict}: ${rec.choice || '(decision)'}`, description: why, ...(taskId ? { taskId } : {}) });
    return rec;
  }
  directorDecisions() {
