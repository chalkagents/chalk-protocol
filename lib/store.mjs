...
const lockGeneration = (lockPath) => {
  try {
    ...
    if (stat.isDirectory()) {
      try {
        const token = readFileSync(join(lockPath, 'owner'), 'utf8').split(' ')[0];
        if (token) return { token, mtimeMs: stat.mtimeMs, kind: 'directory', owned: true };
  ...
  withLock(fn) {
    ...
    for (;;) {
      try {
        ...
        writeFileSync(join(lockPath, 'owner'), `${token} ${now()}`, { flag: 'wx' });
        held = true;
        break;
      }
      catch (e) {
