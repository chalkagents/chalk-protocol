...
export class Store {
  ...
  // tasks.json — last writer wins, silently dropping the other's update. Atomic directory creation is
  // the portable cross-process create-if-absent primitive: whoever wins holds the lock; the rest poll,
  ...
  withLock(fn) {
    ...
    for (;;) {
      try {
        mkdirSync(lockPath);
        held = true;
        try { writeFileSync(join(lockPath, 'owner'), `${token} ${now()}`); }
        catch (e) { rmSync(lockPath, { recursive: true, force: true }); held = false; throw e; }
        break;
      }
      catch (e) {
        const contention = e.code === 'EEXIST'
          || (process.platform === 'win32' && ['EACCES', 'EPERM', 'EBUSY'].includes(e.code) && existsSync(lockPath));
        if (!contention) throw e;
        let stale = false;
        try { stale = Date.now() - statSync(lockPath).mtimeMs > LOCK_STALE_MS; }
        catch (statError) {
          if (statError.code === 'ENOENT') continue; // released between mkdir and stat — retry now
          if (Date.now() - start > LOCK_WAIT_MS) throw e;
        ...
        if (stale) {
          ...
          // lock artifact; the loser's rename ENOENTs and it simply retries the create.
          try { const captured = `${lockPath}.stale.${process.pid}.${Date.now()}`; renameSync(lockPath, captured); rmSync(captured, { recursive: true, force: true }); }
    ...
    try { return fn(); }
      ...
      if (held) {
        // If this holder was declared stale and the path was replaced, never delete the NEW owner's
        // lock during our cleanup. The unique marker makes ownership explicit across rename/recreate.
        try { if (readFileSync(join(lockPath, 'owner'), 'utf8').split(' ')[0] === token) rmSync(lockPath, { recursive: true, force: true }); }
