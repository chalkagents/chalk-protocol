...
export function buildContext(store, task) {
  ...
  if (task) {
    ...
    if (lastReview && lastReview.verdict === 'block' && (lastReview.findings || []).length) {
      ...
      for (const f of lastReview.findings) out.push(`  - [${f.severity || '?'}/${f.area || '?'}] ${f.note || ''}`);
    }
  }
...
export function initSpine(root, { name, goal, preset, runner, executor }) {
  ...
  const meta = {
    ...
    protocol: {
      ...
      github: { command: 'gh', base: 'main', repo: '', mergeMethod: 'squash', labelType: { bug: 'fix', enhancement: 'feat', documentation: 'docs' }, ciPollIntervalMs: 5000, ciPollAttempts: 24 },
