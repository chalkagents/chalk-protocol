...
function directorLines(decisions) {
  return decisions.map((r) => (r.verdict === 'redirected'
    ? `- redirected: "${r.choice || 'a choice'}" → ${r.instruction || '(see record)'}`
    : `- accepted: "${r.choice || 'a choice'}"${r.rationale ? ` (because ${r.rationale})` : ''}`));
}
