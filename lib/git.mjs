...
export function git(cwd, args, { input, timeout = TIMEOUT } = {}) {
  return execSync(`git ${args}`, { cwd, input, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], timeout }).trim();
}
...
export function gitCommitPaths(cwd, message, paths) {
  ...
  const spec = list.map(q).join(' ');
  if (!gitTry(cwd, `status --porcelain -- ${spec}`).trim()) return false;
  ...
  git(cwd, `add -- ${spec}`);
  git(cwd, `commit -m ${q(message)} -- ${spec}`);
  return true;
}
...
export function diffPaths(cwd, base = 'main') {
  return gitTry(cwd, `diff --name-only ${base}...HEAD`).split('\n').filter(Boolean);
}
