...
import { now, protocolPath, workdir } from './store.mjs';
import { changedPaths } from './git.mjs';
import { HANDOFF_TITLE } from './markers.mjs';
...
// any failure (nonzero exit, timeout, missing binary) falls back to template-only — but LOUDLY, so a
// typo'd handoff.command doesn't silently produce empty Notes forever.
function narrate(store, task, reason, changed) {
  const profile = resolveAgentRole(store.protocol(), 'handoff');
  ...
  const agentResult = runAgent('handoff', {
    profile, cwd: workdir(store, task), context: prompt, output: { kind: 'text' }, stderr: 'ignore',
    cost: { store, taskId: task.id },
  });
  if (agentResult.status === 'ok') return agentResult.text.trim();
  const why = agentResult.diagnostics[0]?.message || agentResult.status;
  console.error(`⚠ handoff narrator failed (${why}) — writing the template-only handoff.`);
  return '';
}
...
export function writeHandoff(store, task, { reason = 'manual', note = '', by = 'agent' } = {}) {
  ...
  const persist = tasks => {
    ...
    if (!current || (current.specRevision || 0) !== (task.specRevision || 0)) throw new Error('specification changed while preparing handoff — reload context and retry');
    let seq = (current.specRevisions || []).reduce((n, r) => Math.max(n, r.invalidated?.handoff?.seq || 0), current.handoff?.seq || 0);
    let rel;
    // History and orphaned files survive pointer invalidation. Never overwrite
    // an earlier handoff when starting the next contract's resumption sequence.
    do { seq++; rel = protocolPath(join(REL_DIR, `${task.id.slice(0, 12)}-${seq}.md`)); }
    while (existsSync(join(store.root, rel)));
    mkdirSync(join(store.root, REL_DIR), { recursive: true });
    writeFileSync(join(store.root, rel), render(current, { reason, note, narrative, changed }), { flag: 'wx' });
    record = { path: rel, at: now(), reason, seq, specRevision: current.specRevision || 0 };
    current.handoff = record;
    task.handoff = record;
    return tasks;
  };
  if (typeof store.mutateTasks === 'function') store.mutateTasks(persist);
  else {
    // Preserve the original upsert-only embedding interface. The canonical
    // filesystem Store always uses its locked transaction above.
    persist([task]);
    store.upsertTask(task);
  }
  store.emitUpdate({ type: 'progress-update', title: HANDOFF_TITLE(task), description: `reason: ${reason}`, actorRole: by, taskId: task.id });
  return record;
}
