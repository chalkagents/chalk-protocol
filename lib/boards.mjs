...
import { isSpec, readSpecId, latestRun } from './e2e.mjs';
...
const STATE_COLUMN = { todo: 'todo', specd: 'todo', 'in-progress': 'in_progress', blocked: 'in_progress', done: 'done' };
...
function cardFor(task, index, ctx = {}) {
  const revalidation = task.state === 'done' && !completionCurrent(task);
  ...
  const card = {
    ...
    title: task.title,
    column: revalidation ? 'todo' : STATE_COLUMN[task.state] || 'todo',
    createdAt: created,
    updatedAt: updated,
    order: index,
  };
  if (criteria.length) card.description = criteria.map((c) => `- ${c.text}`).join('\n');
  if (revalidation) card.description = `${card.description || ''}\n\nCurrent contract needs revalidation: chalk start ${task.id}`.trim();

  // testArtifact — the execution view: the locked acceptance test + last run + the PR. lastRun
  // prefers REAL chalk-browser run evidence (the run.json a locked .test.yaml produced — one
  // authoritative source shared with chalk-browser); otherwise it's derived honestly from the
  // gates: `done` means verify was green (P4), else the latest review verdict (P5).
  const specPath = tests.length ? tests[0].path : undefined;
  const lastReview = reviews[reviews.length - 1];
  let lastRun;
  if (completionCurrent(task)) lastRun = { runId: `done-${task.id}`, status: 'passed', at: updated };
  else if (lastReview && !revalidation) lastRun = { runId: `review-${task.id}`, status: lastReview.verdict === 'pass' ? 'passed' : 'failed', at: ms(lastReview.at, updated) };

  let specId;
  if (ctx.root && specPath && isSpec(specPath, ctx.specPattern)) {
    specId = readSpecId(ctx.root, specPath) || undefined;
    const run = specId && latestRun(ctx.root, ctx.runsDir || '.chalk/runs', specId);
    if (run && !revalidation) lastRun = { runId: run.runId, status: run.status, at: Number(run.finishedAt || run.startedAt) || updated }; // real evidence wins
  }

  if (specPath || lastRun || task.pr?.number) {
    card.testArtifact = {};
    if (specPath) card.testArtifact.specPath = specPath;
    if (specId) card.testArtifact.specId = specId;
    if (lastRun) card.testArtifact.lastRun = lastRun;
    if (task.pr?.number) { card.testArtifact.prNumber = task.pr.number; if (task.pr.url) card.testArtifact.prUrl = task.pr.url; }
    if (task.issue?.url) card.testArtifact.evidenceUrl = task.issue.url;
  }
  return card;
}
...
export function projectBoard(store) {
  ...
  const ctx = { root: store.root, runsDir: store.protocol().e2e?.runsDir || '.chalk/runs', specPattern: store.protocol().e2e?.specPattern };
  const cards = store.tasks().map((t, i) => cardFor(t, i, ctx));
  const board = {
    id: boardId(name),
