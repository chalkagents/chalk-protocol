...
function cardFor(task, index, ctx = {}) {
  ...
  let lastRun;
  if (task.state === 'done') lastRun = { runId: `done-${task.id}`, status: 'passed', at: updated };
  else if (lastReview) lastRun = { runId: `review-${task.id}`, status: lastReview.verdict === 'pass' ? 'passed' : 'failed', at: ms(lastReview.at, updated) };
  ...
  if (ctx.root && specPath && isSpec(specPath, ctx.specPattern)) {
    ...
    if (run) lastRun = { runId: run.runId, status: run.status, at: Number(run.finishedAt || run.startedAt) || updated }; // real evidence wins
  }
