...
export function runDiscovery(store, brief) {
  const profile = resolveAgentRole(store.protocol(), 'discovery');
  if (!profile?.command) return { status: 'unconfigured' };
  const { text } = runAgent('discovery', {
    profile, cwd: store.root, input: brief, output: { kind: 'text' }, stderr: 'capture',
