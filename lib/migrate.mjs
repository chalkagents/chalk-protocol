...
export function runMigrate(store, { dryRun = false } = {}) {
  ...
  const route = [from, ...plan.map((step) => step.to)].join(' → ');
  store.appendDecision({ title: `Migrated spine ${route}`, why: `chalk migrate: ${plan.map((p) => p.describe).join('; ')}${backup ? ` (backup: ${backup})` : ''}` });
