...
export function brokeCheck(store, task, { verifyFn = verify, classify = ciStatus, sleep = realSleep } = {}) {
  ...
  if (ci === 'pass' && !task.pipeline?.verificationInvalidated) return { ok: true, source: 'ci', detail: 'remote CI green' };
  ...
  return { ok: !!v.green, source: 'local', contractRevision, detail: v.green ? 'local verify green' : 'local verify is not green — run chalk verify' };
}
