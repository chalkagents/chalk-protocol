...
// safe. Existing remote CI must be green; local verification binds the source and
// contract being admitted, since a remote bucket alone carries no such identity.
...
// (bounded poll — a racing PR isn't "broken"), then verify the local inputs.
// Returns { ok, source: 'ci'|'local', detail }.
export function brokeCheck(store, task, { verifyFn = verify, classify = ciStatus, sleep = realSleep } = {}) {
  ...
  // Bare remote buckets carry no local source/spec/config identity. Green CI is
  // retained as a prerequisite; local verification establishes current inputs.
  ...
  const contractRevision = task.specRevision || 0;
  if (store.task && (store.task(task.id)?.specRevision || 0) !== contractRevision) return { ok: false, source: 'local', detail: 'specification changed — reload context and retry merge' };
  return { ok: !!v.green, source: 'local', ci, contractRevision, approval: v.approvals?.[task.id], detail: v.green ? 'local verify green' : 'local verify is not green — run chalk verify' };
}
